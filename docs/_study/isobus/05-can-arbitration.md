...
**핵심 차이**: 이더넷은 충돌 후 **모두 재전송**하지만, CAN은 중재 패배 노드만 재전송한다. 승리한 노드는 전송을 **중단 없이 계속**한다.
...
비트 8에서 노드A는 0(Dominant)을, 노드B는 1(Recessive)을 전송한다. 버스는 Dominant 값인 0이 된다. 노드B는 자신이 1을 보냈는데 버스가 0이 된 것을 감지하고, **즉시 전송을 중단**한다. 노드A는 아무것도 모른 채 전송을 계속한다.
...
중재 메커니즘을 이해하면 **ID 배정 전략**이 명확해진다.
