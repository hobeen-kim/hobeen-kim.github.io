...
flowchart TB
    ...
    ALLOY["Grafana Alloy\n(Collector 임베드 + Alloy 구문 +\nprometheus.*/loki.*/pyroscope.* 네이티브 컴포넌트)"]
    AGENT["Grafana Agent\n(EOL, Alloy로 이관 완료)"]

    OTEL --> CONTRIB
    OTEL -->|"코드베이스 임베드"| ALLOY
    AGENT -.->|"후속"| ALLOY
```
...
```mermaid
flowchart TD
    START["수집 계층 선택"] --> Q1{"백엔드가\nGrafana LGTM+ 스택?"}
    Q1 -->|"Yes"| Q2{"벤더 락인 회피가\n최우선 정책?"}
    Q1 -->|"No"| COLLECTOR1["OTel Collector\n(순수, 벤더 중립)"]
    Q2 -->|"Yes"| COLLECTOR2["OTel Collector\n+ Grafana OTLP 익스포터"]
    Q2 -->|"No"| Q3{"clustering·프로파일\n네이티브 지원 필요?"}
    Q3 -->|"Yes"| ALLOY["Alloy"]
    Q3 -->|"No"| EITHER["둘 다 가능,\n팀 숙련도로 결정"]
```
