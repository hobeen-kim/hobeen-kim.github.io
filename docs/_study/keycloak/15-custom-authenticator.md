...
prev: /study/keycloak/14-spi-overview
next: /study/keycloak/16-custom-user-storage
---
...
Factory는 서버당 싱글톤이므로 외부 API 클라이언트·커넥션 풀·설정 캐시를 보관한다. Authenticator는 요청마다 새로 만들어져 Factory가 초기화해 둔 자원을 쓴다. [CH14](/study/keycloak/14-spi-overview)에서 정리한 Factory/Provider 관계가 그대로 적용된다.
...
- `template.ftl`은 [CH17. Theme 커스터마이징](/study/keycloak/17-theme)에서 다룰 공통 레이아웃이다.
...
[CH9. Authentication Flow](/study/keycloak/09-auth-flow)에서 다룬 Flow 구조에 새 Authenticator를 끼워 넣어야 실제로 동작한다.
...
- Flow 배치는 [CH9](/study/keycloak/09-auth-flow)의 Required/Alternative 규칙을 따르며 Conditional 서브플로우로 조건부 적용이 가능하다.
:::

## 다음 챕터

- 이전 : [SPI로 Keycloak 확장](/study/keycloak/14-spi-overview)
- 다음 : [커스텀 User Storage](/study/keycloak/16-custom-user-storage)
