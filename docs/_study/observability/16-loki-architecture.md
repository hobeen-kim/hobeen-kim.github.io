...
```mermaid
flowchart LR
    subgraph Prom["Prometheus 모델"]
        PM["metric + labels\n(숫자 값 자체를 인덱싱)"]
    end
    subgraph LokiModel["Loki 모델"]
        LL["labels\n(인덱스 대상)"]
        LC["log line\n(압축 청크, 비인덱싱)"]
        LL -->|"스트림 식별"| LC
    end
...
flowchart TB
    ...
    FILTER["라인 필터 / 파서 적용"]
    Q -->|"1. 라벨 매처로 조회"| IDX
    IDX -->|"2. 청크 참조 목록"| CHUNKS
    CHUNKS -->|"3. 실제 데이터 fetch"| OBJ
    CHUNKS --> FILTER
    FILTER --> RESULT["결과 반환"]
```
...
```mermaid
flowchart LR
    subgraph Good["안전한 라벨 설계"]
        L1["app, env, namespace, level\n(값 개수 적음)"]
        S1["스트림 수: 수백~수천"]
        L1 --> S1
    end
    subgraph Bad["위험한 라벨 설계"]
        L2["request_id, user_id, trace_id\n(값 개수 무한대)"]
        S2["스트림 수: 무한 증가"]
        L2 --> S2
    end
```
...
flowchart TB
    ...
    subgraph Simple["Simple Scalable"]
