...
prev: /study/keycloak/03-client-service-account
next: /study/keycloak/05-role-group
---
...
레거시 시스템에서 BCrypt로 저장된 해시를 그대로 Keycloak으로 옮기려면 별도 Credential Provider SPI가 필요하다(CH14~17). 단순 이관이 아니라 "레거시 해시를 검증만 하다가, 로그인 성공 시 Keycloak 표준 해시로 재해싱"하는 패턴이 실무 표준이다.
...
- Recovery Codes 기능 활성화(CH11에서 상세).
...
- 이전 : [Client와 Service Account](/study/keycloak/03-client-service-account)
- 다음 : [Role·Group과 Composite Role](/study/keycloak/05-role-group)
