...
prev: /study/keycloak/07-authz-uma
next: /study/keycloak/09-auth-flow
---
...
외부 IdP로 처음 로그인한 사용자를 Keycloak 로컬 계정과 어떻게 연결할지는 [CH9. 인증 플로우](/study/keycloak/09-auth-flow)에서 설명할 **First Broker Login** 플로우가 결정한다. 기본 동작은 다음과 같다.

1. 이메일이 기존 사용자와 같으면 "기존 계정에 연결" 선택지를 제시.
2. 없으면 새 사용자로 생성(프로비저닝).

브로커링 설계의 자세한 분류는 [CH13. Identity Brokering](/study/keycloak/13-identity-brokering) 예고.
...
- 이전 : [Authorization Services와 UMA 2.0](/study/keycloak/07-authz-uma)
- 다음 : [인증 플로우 커스터마이징](/study/keycloak/09-auth-flow)
