...
prev: /study/keycloak/04-user-credentials
next: /study/keycloak/06-protocol-mapper
---
...
토큰에 그룹 경로를 실으려면 <strong>Group Membership Mapper</strong>를 추가한다(CH6에서 상세).
...
Role·Group만으로는 "alice가 프로젝트 X의 편집자, 프로젝트 Y의 조회자"처럼 <strong>리소스 인스턴스별 권한</strong>을 표현하기 어렵다. 이럴 때 Keycloak Authorization Services(UMA)로 넘어간다. 상세는 CH7에서 다룬다.
...
- 이전 : [사용자와 자격 증명](/study/keycloak/04-user-credentials)
- 다음 : [Client Scope와 Protocol Mapper](/study/keycloak/06-protocol-mapper)
