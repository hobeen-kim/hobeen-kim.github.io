import{_ as s,c as a,f as e,o as t}from"./app-taZ-kvYT.js";const l={};function p(i,n){return t(),a("div",null,n[0]||(n[0]=[e(`<h1 id="에이전트-정의-파일-작성법" tabindex="-1"><a class="header-anchor" href="#에이전트-정의-파일-작성법"><span>에이전트 정의 파일 작성법</span></a></h1><div class="hint-container info"><p class="hint-container-title">학습 목표</p><ul><li>에이전트 정의 파일의 역할과 구조를 이해한다.</li><li>&quot;항상 일하고, 항상 생각하고, 두 번 생각하도록&quot; 원칙을 정의 파일에 내장하는 방법을 익힌다.</li><li>프로젝트 특화 CLAUDE.md와의 관계를 파악한다.</li><li>역할별 정의 파일 예시를 작성할 수 있다.</li></ul></div><h2 id="_1-왜-정의-파일이-필요한가" tabindex="-1"><a class="header-anchor" href="#_1-왜-정의-파일이-필요한가"><span>1. 왜 정의 파일이 필요한가</span></a></h2><p>에이전트는 새 세션을 시작할 때 컨텍스트가 전혀 없다. 오케스트레이터가 에이전트를 생성하면서 프롬프트를 전달하지만, 그 프롬프트에는 &quot;무엇을 하라&quot;는 지시만 있을 뿐 &quot;어떻게 행동해야 하는가&quot;는 담겨 있지 않다.</p><p>정의 파일은 에이전트의 헌법이다. 역할, 원칙, 금지사항을 명시하여 에이전트가 매번 같은 기준으로 판단하게 만든다.</p><pre class="mermaid">flowchart TD
    ...
    style C fill:#e8f4e8
    style D fill:#e8f4e8
</pre><p>oh-my-claudecode 패턴은 YAML frontmatter + 마크다운 본문으로 정의 파일을 구성한다. frontmatter는 레지스트리와 훅이 읽고, 본문은 에이전트가 직접 읽어 행동 기준으로 삼는다.</p><h2 id="_2-정의-파일-형식" tabindex="-1"><a class="header-anchor" href="#_2-정의-파일-형식"><span>2. 정의 파일 형식</span></a></h2><div class="language-yaml line-numbers-mode" data-highlighter="prismjs" data-ext="yml" data-title="yml"><pre><code><span class="line"><span class="token punctuation">---</span></span>
...
<span class="line"></span></code></pre><div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0;"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>각 frontmatter 필드의 역할은 다음과 같다.</p><table><thead><tr><th>필드</th><th>설명</th></tr></thead><tbody><tr><td><code>name</code></td><td>레지스트리 키. <code>agentType</code>으로 사용된다.</td></tr><tr><td><code>description</code></td><td>오케스트레이터가 적절한 에이전트를 선택할 때 참조한다.</td></tr><tr><td><code>model</code></td><td><code>haiku</code> / <code>sonnet</code> / <code>opus</code> 중 하나. 작업 복잡도에 맞춰 선택한다.</td></tr><tr><td><code>team</code></td><td>소속 팀. 레지스트리 파티션으로 작동한다.</td></tr><tr><td><code>disallowedTools</code></td><td>이 에이전트가 사용해서는 안 되는 도구 목록.</td></tr></tbody></table><p><code>disallowedTools</code>는 역할 경계를 강제하는 핵심 필드다. <code>dev</code> 에이전트가 <code>TeamCreate</code>를 호출하는 것은 역할 침범이므로 목록에 추가한다.</p><h2 id="_3-항상-일하고-항상-생각하고-두-번-생각하도록-내장" tabindex="-1"><a class="header-anchor" href="#_3-항상-일하고-항상-생각하고-두-번-생각하도록-내장"><span>3. &quot;항상 일하고, 항상 생각하고, 두 번 생각하도록&quot; 내장</span></a></h2><p>모든 에이전트 정의 파일은 아래 공통 섹션을 본문에 포함한다.</p><div class="language-markdown line-numbers-mode" data-highlighter="prismjs" data-ext="md" data-title="md"><pre><code><span class="line"><span class="token title important"><span class="token punctuation">#</span> 행동 원칙 (반드시 지킨다)</span></span>
...
<span class="line"></span></code></pre><div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0;"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>세 원칙의 관계를 정리하면 다음과 같다.</p><pre class="mermaid">flowchart LR
    ...
    A2 --&gt; B2
    B2 --&gt; C2
</pre><p>세 원칙은 순서대로 작동한다. 먼저 시작하고, 진행하면서 전제를 검증하고, 완료 전에 다시 한 번 검토한다.</p><h2 id="_4-역할별-정의-파일-예시" tabindex="-1"><a class="header-anchor" href="#_4-역할별-정의-파일-예시"><span>4. 역할별 정의 파일 예시</span></a></h2><details class="hint-container details"><summary>orchestrator.md — 전체 예시</summary><div class="language-markdown line-numbers-mode" data-highlighter="prismjs" data-ext="md" data-title="md"><pre><code><span class="line"><span class="token front-matter-block"><span class="token punctuation">---</span></span>
...
<span class="line"></span></code></pre><div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0;"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>두 파일의 읽기 순서와 관계는 다음과 같다.</p><pre class="mermaid">flowchart TD
    ...
    style B fill:#dce8ff
    style C fill:#ffe8dc
</pre><p>CLAUDE.md가 정의 파일보다 나중에 읽히므로, 프로젝트 특화 규칙이 보편 원칙을 덮어쓸 수 있다. 충돌이 발생하면 CLAUDE.md 규칙이 우선한다.</p><div class="hint-container warning"><p class="hint-container-title">정의 파일과 CLAUDE.md 충돌 방지</p><p><code>agents/dev.md</code>에 &quot;TypeScript를 사용한다&quot;고 명시했더라도 CLAUDE.md에 &quot;이 프로젝트는 JavaScript&quot;라고 적혀 있으면 JavaScript가 우선된다. 의도하지 않은 덮어쓰기를 막으려면 두 파일의 범위를 명확히 분리해야 한다.</p></div><h2 id="_6-sessionstart-훅으로-자동-로드" tabindex="-1"><a class="header-anchor" href="#_6-sessionstart-훅으로-자동-로드"><span>6. SessionStart 훅으로 자동 로드</span></a></h2><p>에이전트가 시작될 때 정의 파일을 자동으로 읽히도록 SessionStart 훅을 설정한다.</p><div class="language-typescript line-numbers-mode" data-highlighter="prismjs" data-ext="ts" data-title="ts"><pre><code><span class="line"><span class="token comment">// hooks/session-start-loader.ts</span></span>
