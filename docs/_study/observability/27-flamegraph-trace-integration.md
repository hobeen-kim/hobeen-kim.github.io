...
flowchart TB
    ROOT["main (100%)"]
    ROOT --> H["handleRequest (95%)"]
    H --> P["parseJSON (40%)"]
    H --> DB["dbQuery (50%)"]
    DB --> ENC["encodeRow (30%)"]
    DB --> CONN["acquireConn (20%)"]
    H --> LOG["writeLog (5%)"]
```
...
```mermaid
flowchart LR
    subgraph Flame["flame graph (전통 방향)"]
        direction BT
        F1["main"] --> F2["handleRequest"] --> F3["dbQuery"]
    end
    subgraph Icicle["icicle (Pyroscope UI 기본)"]
        direction TB
        I1["main"] --> I2["handleRequest"] --> I3["dbQuery"]
    end
```
...
```mermaid
sequenceDiagram
    participant OTel as OTel SDK (트레이싱)
    participant App as 애플리케이션 코드
    participant Pyro as Pyroscope SDK (프로파일링)

    OTel->>App: span 시작 (span_id=abc123)
    App->>Pyro: 실행 중 프로파일 샘플에\nspan_id=abc123 태그 부착
    App->>App: 비즈니스 로직 실행
    OTel->>App: span 종료
    Note over Pyro: 이 구간의 샘플만\nspan_id=abc123으로 필터 가능
```
