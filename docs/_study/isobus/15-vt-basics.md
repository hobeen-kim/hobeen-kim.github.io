...
description: "ISOBUS Virtual Terminal의 개념, 동작 원리, 버전별 기능, 그리고 AUX 보조 입력까지 VT의 기초를 이해합니다."
date: 2026-04-13
...
<strong>Virtual Terminal(VT)</strong>은 ISO 11783-6에 정의된 표준 사용자 인터페이스 시스템입니다. 트랙터 운전석에 장착된 디스플레이에 **작업기의 UI를 표시하고 사용자 입력을 받는** 인터페이스 역할을 합니다.
...
즉, <strong>디스플레이 하드웨어는 트랙터가 제공</strong>하고, <strong>화면에 무엇을 보여줄지는 작업기 ECU가 결정</strong>합니다. 트랙터와 작업기의 역할이 명확히 분리되어 있는 것이 핵심입니다.
...
VT 표준이 없던 시절, 각 작업기 제조사는 <strong>전용 디스플레이 패널</strong>을 함께 납품했습니다.

- 비료 살포기 → 비료 살포기 전용 컨트롤 박스
- 파종기 → 파종기 전용 모니터
- 스프레이어 → 스프레이어 전용 디스플레이

운전석에 작업기가 늘어날수록 디스플레이도 늘어났습니다. 배선은 복잡해지고, 운전자는 여러 개의 서로 다른 UI를 익혀야 했습니다.

### VT 이후: 통합 디스플레이

VT는 이 문제를 <strong>스마트폰 앱 스토어 모델</strong>로 해결했습니다.
...
하나의 VT 디스플레이에 어떤 제조사의 작업기를 연결해도, 작업기가 자신의 UI 정의를 VT로 전송하면 VT가 화면을 렌더링합니다. 운전자는 <strong>하나의 디스플레이로 모든 작업기를 조작</strong>할 수 있습니다.
...
VT와 작업기 ECU 사이의 동작은 크게 세 단계로 나뉩니다.
...
VT Server와 Client가 서로 다른 버전을 지원할 경우, <strong>낮은 버전으로 협상</strong>하여 동작합니다. 협상 과정은 VT Status 메시지 교환 시 이루어집니다.
...
<strong>AUX(Auxiliary Input)</strong>는 조이스틱, 추가 버튼 패드, 풋 페달 등 VT 디스플레이 외부의 보조 입력 장치를 ISOBUS에 통합하는 메커니즘입니다. ISO 11783-6 Annex에 정의되어 있습니다.

현재 표준에서 사용되는 버전은 **AUX-N**(New AUX)으로, 기존 AUX-O(Old AUX)를 대체했습니다.
...
AUX의 핵심 개념 중 하나는 <strong>Preferred Assignment</strong>입니다. 사용자가 한 번 입력 장치와 기능의 매핑을 설정하면, 그 설정이 VT에 저장됩니다. 다음에 같은 작업기를 연결하면 <strong>이전 매핑이 자동으로 복원</strong>됩니다.
...
::: tip 핵심 정리
- VT는 ISO 11783-6에 정의된 표준 디스플레이 인터페이스로, 트랙터 디스플레이(VT Server)와 작업기 ECU(VT Client)로 구성됩니다.
- 작업기 ECU는 오브젝트 풀을 VT로 전송하고, VT는 이를 렌더링합니다.
- VT는 버전(2~6)에 따라 지원 기능이 다르며, 낮은 버전으로 자동 협상합니다.
- AUX-N은 조이스틱 등 보조 입력 장치를 ISOBUS에 통합하며, Preferred Assignment로 매핑 설정을 저장합니다.
:::
