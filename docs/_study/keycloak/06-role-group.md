...
prev: /study/keycloak/05-user-credentials
next: /study/keycloak/07-protocol-mapper
---
...
| 대표 예 | 사용자 정의 Realm Role 예시: `shop-user`, `shop-staff`, `shop-admin` (`realm-admin`은 master realm의 내장 역할이므로 혼동 주의) | `myapp:editor`, `myapp:viewer` |
...
이 구조는 Spring Security Resource Server에서 권한 매핑 시 자주 문제가 된다. 기본 매퍼는 `SCOPE_*`만 GrantedAuthority로 변환한다. Keycloak Role을 쓰려면 `JwtGrantedAuthoritiesConverter`를 교체해 `realm_access.roles`와 `resource_access.*.roles`를 읽는 커스텀 컨버터를 붙인다. 자세한 패턴은 [OAuth 스터디 CH16](/study/oauth/16-spring-security)에서 다룬다.
...
토큰에 그룹 경로를 실으려면 <strong>Group Membership Mapper</strong>를 추가한다(CH7에서 상세).
...
<strong>Realm roles</strong> 목록에서 `default-roles-<realm>` Composite Role을 직접 편집하여 관리한다. 신규 User는 이 역할의 하위 역할을 자동으로 상속받는다. Default Roles는 Composite Role의 대표 사례다. 기본적으로 `default-roles-<realm>`이라는 이름의 Composite이 존재하며, 모든 사용자에게 자동 부여된다. 새 Realm에 기본 권한을 심을 때 이 Composite을 편집한다.
...
- Admin REST API에서는 `/admin/realms/{realm}/users/{id}/role-mappings/realm/composite`와 `/admin/realms/{realm}/users/{id}/role-mappings/clients/{client-uuid}/composite` (`client-uuid`는 Client의 UUID이며 Client ID 문자열이 아님) 엔드포인트로 같은 정보를 얻는다.
...
Clients → 해당 Client → <strong>Client scopes</strong> 탭 → 스코프 목록 상단의 <strong>Full scope allowed</strong> 토글 (또는 Client 상세 화면의 Scope 섹션).
...
Role·Group만으로는 "alice가 프로젝트 X의 편집자, 프로젝트 Y의 조회자"처럼 <strong>리소스 인스턴스별 권한</strong>을 표현하기 어렵다. 이럴 때 Keycloak Authorization Services(UMA)로 넘어간다. 상세는 CH8에서 다룬다.
...
- 이전 : [사용자와 자격 증명](/study/keycloak/05-user-credentials)
- 다음 : [Client Scope와 Protocol Mapper](/study/keycloak/07-protocol-mapper)
