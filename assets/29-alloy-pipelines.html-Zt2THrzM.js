...
</pre><h2 id="_3-트레이스-파이프라인-—-otelcol-receiver-→-processor-→-exporter" tabindex="-1"><a class="header-anchor" href="#_3-트레이스-파이프라인-—-otelcol-receiver-→-processor-→-exporter"><span>3. 트레이스 파이프라인 — otelcol.receiver → processor → exporter</span></a></h2><p>트레이스는 <code>otelcol.*</code> 계열로 구성한다. <a href="/study/observability/21-opentelemetry">OpenTelemetry</a> SDK가 보낸 OTLP를 <code>otelcol.receiver.otlp</code>가 받고, 프로세서 체인을 거쳐 <code>otelcol.exporter.otlp</code>가 Tempo로 내보낸다.</p><div class="language-alloy line-numbers-mode" data-highlighter="prismjs" data-ext="alloy" data-title="alloy"><pre><code><span class="line">otelcol.receiver.otlp &quot;default&quot; {</span>
...
<span class="line">  }</span>
<span class="line">}</span>
<span class="line"></span></code></pre><div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0;"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><pre class="mermaid">sequenceDiagram
