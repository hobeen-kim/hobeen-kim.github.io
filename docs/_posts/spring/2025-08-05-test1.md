...
fun <T> request(
        ...
        val result = mockMvc.perform(
            mockHttpServletRequestBuilder(method, url)
                .headers(headers)
        ...
        return objectMapper.readValue(result.response.contentAsString, returnTypeRef)
}

    private fun mockHttpServletRequestBuilder(method: HttpMethod, url: String): MockHttpServletRequestBuilder {
        when (method) {
            HttpMethod.GET -> return get(url)
            HttpMethod.POST -> return post(url)
            HttpMethod.PUT -> return put(url)
            HttpMethod.PATCH -> return patch(url)
            HttpMethod.DELETE -> return delete(url)
        }

        throw IllegalArgumentException("HttpMethod ${method.name()} is not supported")
    }
```
