...
prev: /study/keycloak/18-ha-clustering
next: /study/keycloak/20-database-performance
---
...
[CH18. Infinispan HA 클러스터링](/study/keycloak/18-ha-clustering)에서 살펴본 클러스터 구성과 `kc.sh start` 옵션을 전부 자동으로 관리해 주는 것이 Operator다. Operator는 사용자가 선언한 원하는 상태(desired state)와 실제 상태(actual state)를 계속 비교하면서 차이를 메꾼다.
...
| `db.poolMaxSize` | Agroal 커넥션 풀 상한 | [CH20](/study/keycloak/20-database-performance) 참조 |
...
`instances: 3`으로 지정하면 Operator가 [CH18](/study/keycloak/18-ha-clustering)에서 본 Infinispan 기반 클러스터를 자동으로 구성한다. Headless Service가 같이 생성되고, JGroups DNS_PING이 자동으로 설정된다.
...
Operator는 Admin REST API([CH21](/study/keycloak/21-admin-rest-api) 참조)를 호출해 Realm을 생성한다. 내부적으로 `master` realm의 관리자 자격증명을 자동 생성 Secret에서 읽어 사용한다.
...
Realm을 CR로 선언하면 Argo CD/Flux 같은 GitOps 도구가 전체 IAM 상태를 관리할 수 있다. [CH21. Admin REST API](/study/keycloak/21-admin-rest-api)에서 Terraform 접근법을 다루는데, Operator + GitOps는 그 첫 번째 대안이다.
...
`instances` 수를 늘리거나 이미지 태그를 바꾸면 Operator가 StatefulSet을 롤링 업데이트한다. 주의할 점은 메이저 버전 업그레이드에서 DB 마이그레이션이 동반된다는 것이다([CH23](/study/keycloak/23-monitoring-upgrade) 참조).
...
- `/metrics` Prometheus 엔드포인트([CH23](/study/keycloak/23-monitoring-upgrade))
...
Operator와 GitOps로 배포 자체는 선언화됐다. 이제 Keycloak이 기대대로 돌려면 백엔드 DB가 병목이 되지 않아야 한다. [CH20. 데이터베이스와 성능](/study/keycloak/20-database-performance)에서 PostgreSQL 기준 Keycloak 스키마, Connection Pool 튜닝, 오프라인 세션 처리, 느린 쿼리 진단을 다룬다.

- 이전: [CH18. Infinispan HA 클러스터링](/study/keycloak/18-ha-clustering)
- 다음: [CH20. 데이터베이스와 성능](/study/keycloak/20-database-performance)
