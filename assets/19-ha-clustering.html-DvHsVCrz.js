...
<span class="line"></span></code></pre><div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0;"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>v26의 <code>persistent-user-sessions</code> 기능과 맞물리면(기본 활성화 여부는 마이너 버전별로 다르므로 릴리스 노트 확인) Keycloak 재기동 시에도 세션이 살아남는다. 이제 세션이 Infinispan 클러스터에 모여 있으므로, Keycloak Pod 하나가 죽어도 사용자는 재로그인할 필요가 없다.</p><h2 id="_5-캐시-동기화-모드" tabindex="-1"><a class="header-anchor" href="#_5-캐시-동기화-모드"><span>5. 캐시 동기화 모드</span></a></h2><p>Infinispan 캐시의 쓰기 전파 방식은 <strong>SYNC</strong>와 <strong>ASYNC</strong> 두 가지다. 선택에 따라 처리량과 정합성이 크게 달라진다.</p><h3 id="sync-모드" tabindex="-1"><a class="header-anchor" href="#sync-모드"><span>SYNC 모드</span></a></h3><pre class="mermaid">sequenceDiagram
    ...
    N1--&gt;&gt;C: 200 OK
    N1-&gt;&gt;N2: Replicate (비동기)
</pre><ul><li>로컬에 쓰고 바로 응답. 복제는 뒤에서 진행.</li><li>응답이 빠르지만, 복제 전에 노드가 죽으면 데이터 유실 가능.</li><li>사용자가 Node 1에 로그인하고 다음 요청이 Node 2로 가면 &quot;세션 없음&quot; 에러가 날 수 있다.</li></ul><h3 id="keycloak-권장-설정" tabindex="-1"><a class="header-anchor" href="#keycloak-권장-설정"><span>Keycloak 권장 설정</span></a></h3><table><thead><tr><th>캐시</th><th>권장 모드</th><th>이유</th></tr></thead><tbody><tr><td><code>sessions</code></td><td>SYNC</td><td>SSO 세션이 누락되면 곧바로 재로그인</td></tr><tr><td><code>authenticationSessions</code></td><td>SYNC</td><td>로그인 진행 중 상태가 사라지면 플로우 실패</td></tr><tr><td><code>offlineSessions</code></td><td>SYNC</td><td>Refresh 토큰 일관성 필요</td></tr><tr><td><code>loginFailures</code></td><td>ASYNC</td><td>카운터 약간 부정확해도 무방</td></tr><tr><td><code>actionTokens</code></td><td>SYNC</td><td>한 번 쓰고 버리는 토큰이라 노드 간 중복·누락이 곧 보안 사고</td></tr></tbody></table><p>로그인 실패 카운터만 ASYNC를 쓰면 약간의 성능 이득이 있지만, 대부분 실무에서는 전부 SYNC로 통일한다. Brute Force 카운터가 노드 간 불일치면 공격자가 노드를 바꿔가며 시도 횟수를 우회할 수 있기 때문이다.</p><h2 id="_6-multi-site-배포" tabindex="-1"><a class="header-anchor" href="#_6-multi-site-배포"><span>6. Multi-site 배포</span></a></h2><p><strong>Multi-site</strong>는 지리적으로 떨어진 두 데이터센터에 Keycloak을 동시에 운영하는 구조다. 단일 DC 장애에도 서비스를 유지하려는 목적이 크다. Keycloak은 v26부터 공식 Multi-site 가이드를 제공하며, 권장 형태는 <strong>Active/Passive</strong>다.</p><h3 id="active-passive-구조" tabindex="-1"><a class="header-anchor" href="#active-passive-구조"><span>Active/Passive 구조</span></a></h3><pre class="mermaid">flowchart LR
