...
::: info 학습 목표
- Keycloak이 노출하는 SPI(Service Provider Interface)의 구조와 주요 확장점 종류를 설명할 수 있다.
...
| 사용자 저장 | UserStorageProvider (+ Capability 인터페이스들) |
...
Keycloak 9.x 이하에 있던 구 `UserFederationProvider` SPI는 UserStorage SPI로 통합되어 26.x에서는 제거됐다.
