...
prev: /study/keycloak/10-auth-flow
next: /study/keycloak/12-mfa
---
...
| 규칙 | 동작 |
|------|------|
| Hash Algorithm | 해시 알고리즘 선택(`argon2id`, `pbkdf2-sha256`, `pbkdf2-sha512`) |
...
비밀번호는 절대 평문으로 저장하면 안 된다. Keycloak 26.x 신규 Realm의 기본 해시는 <strong>Argon2id</strong>다. 과거 기본이었던 PBKDF2-SHA512는 기존 Realm·호환 환경에서 여전히 쓰인다.
...
PBKDF2-SHA512의 기본 반복 횟수는 버전별로 상향되어 왔다. 구체적 수치는 해당 버전의 `PasswordPolicyProvider` 기본값 또는 공식 릴리스 노트를 확인한다. 반복 횟수를 올릴수록 무차별 대입 비용이 커지는 만큼, 정책 업데이트와 함께 CPU 사용량을 모니터링해야 한다([CH21. DB 성능](/study/keycloak/21-database-performance)과 함께 본다).
...
- 백업에는 해시만 들어 있더라도 유출 시 오프라인 공격이 가능 — [CH23. Backup](/study/keycloak/23-backup-restore)에서 다룰 암호화 저장 필수.
...
|------|------|
| Enabled | 탐지 on/off |
| Detection Mode | Permanent/Temporary 중 선택 — 아래 Permanent Lockout vs Temporary Lockout 참조 |
...
CH22에서 다룰 Admin REST API와 결합하면 전체 파이프라인을 코드로 운영할 수 있다. 예: Slack 슬래시 커맨드 → 서비스 계정 토큰 → Keycloak 해제 API 호출 → 감사 로그. 자세한 예제는 [CH22. Admin REST API](/study/keycloak/22-admin-rest-api)로 이어진다.
...
- 이전 : [인증 플로우 커스터마이징](/study/keycloak/10-auth-flow)
- 다음 : [MFA — TOTP / WebAuthn](/study/keycloak/12-mfa)
