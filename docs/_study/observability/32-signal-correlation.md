...
```mermaid
flowchart LR
    subgraph Isolated["신호가 고립된 경우"]
        M1["메트릭 이상 발견"] -->|"수동으로 시간·서비스 맞춰\n로그 검색"| L1["로그 확인"]
        L1 -->|"수동으로 trace ID 복사"| T1["트레이스 조회"]
    end
    subgraph Correlated["상관관계가 연결된 경우"]
        M2["메트릭 이상 발견"] -->|"exemplar 클릭"| T2["트레이스"]
        T2 -->|"trace-to-logs"| L2["로그"]
        T2 -->|"trace-to-profiles"| P2["플레임그래프"]
    end
```
...
sequenceDiagram
    ...
    App->>Prom: 히스토그램 관측값 + exemplar(trace_id)
    Note over Prom: exemplar-storage에 별도 보관
    Graf->>Prom: PromQL 쿼리
    Prom-->>Graf: 시계열 + exemplar 포인트
    Note over Graf: 그래프 위 점으로 렌더링
    Graf->>Tempo: 사용자가 점 클릭 → trace_id로 조회
    Tempo-->>Graf: 트레이스 반환
```
...
sequenceDiagram
    ...
    participant Dash as RED 대시보드
    participant Tempo as Tempo
    participant Loki as Loki
    participant Pyro as Pyroscope

    SRE->>Dash: p99 레이턴시 스파이크 확인
    Dash-->>SRE: exemplar 점 표시
    SRE->>Tempo: exemplar 클릭 → 트레이스 조회
    Tempo-->>SRE: 느린 span 확인 (특정 DB 호출)
    SRE->>Loki: "Logs for this span" 클릭
    Loki-->>SRE: 해당 구간 에러 로그 없음 (정상 응답, 단순 지연)
    SRE->>Pyro: "Profiles for this span" 클릭
    Pyro-->>SRE: 플레임그래프에서 GC 시간 비중 급증 확인
    Note over SRE: 근본 원인 = GC 압박, 애플리케이션 버그 아님
```
