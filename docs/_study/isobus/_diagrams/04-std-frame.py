"""CH4 §2 CAN Standard Data Frame (2.0A) 필드 배치 (light/dark PNG)."""
import math
...
def draw(d):
    ...
    for (name, n, key, lvl), w in zip(FIELDS, widths):
        ...
        # 필드 이름을 위쪽 브래킷으로 연결 (레벨 교대로 겹침 방지)
        ly = Y + H + (1.6 if lvl == 0 else 4.6)
        ...
        d.text(x + w / 2, ly + 1.35, name, size=8.3)
...
diagram("04-std-frame", draw, w=13.5, h=4.6, xmax=100, ymax=30)
