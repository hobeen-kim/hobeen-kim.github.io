...
prev: /study/keycloak/02-realm-organizations
next: /study/keycloak/04-user-credentials
---
...
- 자동화는 Admin REST API(CH21)와 CI/CD로 구성한다.
...
| Authorization | Keycloak Authorization Services (UMA, CH7) |
...
4. <strong>Roles</strong>: 이 Client에 귀속된 Client Role 정의(CH5).
5. <strong>Client scopes</strong>: 기본/선택 스코프 매핑(CH6).
6. <strong>Sessions</strong>: 이 Client의 활성 세션.
7. <strong>Advanced</strong>: Consent·Logout·Lifespan 오버라이드.
8. <strong>Authorization</strong>: Authorization Services 켠 경우에만 표시(CH7).
...
- 이전 : [Realm과 Organizations](/study/keycloak/02-realm-organizations)
- 다음 : [사용자와 자격 증명](/study/keycloak/04-user-credentials)
