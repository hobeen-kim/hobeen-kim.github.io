...
![SLI(측정값)에서 SLO(내부 목표)를 거쳐 SLA(고객 계약)로 이어지는 세 층위와 각 층의 정의·여유 관계](/images/study-observability/15-sli-slo-sla-light.png)
![SLI(측정값)에서 SLO(내부 목표)를 거쳐 SLA(고객 계약)로 이어지는 세 층위와 각 층의 정의·여유 관계](/images/study-observability/15-sli-slo-sla-dark.png)
...
![긴 윈도 조건과 짧은 윈도 조건을 AND로 묶어 둘 다 참일 때만 알림을 발화하는 로직과, 단일 윈도만 감시할 때의 반응 지연·오탐 문제](/images/study-observability/15-burn-rate-and-light.png)
![긴 윈도 조건과 짧은 윈도 조건을 AND로 묶어 둘 다 참일 때만 알림을 발화하는 로직과, 단일 윈도만 감시할 때의 반응 지연·오탐 문제](/images/study-observability/15-burn-rate-and-dark.png)
