...
<span class="line">  }</span>
<span class="line">}</span>
<span class="line"></span></code></pre><div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0;"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p><code>stage.json</code>으로 JSON 로그를 파싱하고, <code>stage.labels</code>로 파싱 결과 중 라벨로 승격할 값만 골라낸다. 라벨은 인덱스 비용을 직접 좌우하므로(<a href="/study/observability/16-loki-architecture">Loki 아키텍처</a> 참고), <code>message</code>처럼 카디널리티가 높은 필드는 절대 라벨로 승격하지 않고 로그 본문에 그대로 둔다. <code>stage.drop</code>처럼 파이프라인 단계에서 노이즈 로그를 걸러내면 Loki에 도달하는 볼륨 자체를 줄여 저장 비용을 낮출 수 있다.</p><p><img src="`+i+`" alt="로그 파이프라인 흐름 — discovery.kubernetes(Pod 로그 소스) → loki.source.kubernetes → loki.process(stage.json → stage.labels → stage.drop) → loki.write → Loki로 이어지는 컴포넌트 체인"></p><h2 id="_3-트레이스-파이프라인-—-otelcol-receiver-→-processor-→-exporter" tabindex="-1"><a class="header-anchor" href="#_3-트레이스-파이프라인-—-otelcol-receiver-→-processor-→-exporter"><span>3. 트레이스 파이프라인 — otelcol.receiver → processor → exporter</span></a></h2><p>트레이스는 <code>otelcol.*</code> 계열로 구성한다. <a href="/study/observability/21-opentelemetry">OpenTelemetry</a> SDK가 보낸 OTLP를 <code>otelcol.receiver.otlp</code>가 받고, 프로세서 체인을 거쳐 <code>otelcol.exporter.otlp</code>가 Tempo로 내보낸다.</p><div class="language-alloy line-numbers-mode" data-highlighter="prismjs" data-ext="alloy" data-title="alloy"><pre><code><span class="line">otelcol.receiver.otlp &quot;default&quot; {</span>
