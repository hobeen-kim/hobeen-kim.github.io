...
prev: /study/keycloak/15-spi-overview
next: /study/keycloak/17-custom-user-storage
---
...
Factory는 서버당 싱글톤이므로 외부 API 클라이언트·커넥션 풀·설정 캐시를 보관한다. Authenticator는 요청마다 새로 만들어져 Factory가 초기화해 둔 자원을 쓴다. [CH15](/study/keycloak/15-spi-overview)에서 정리한 Factory/Provider 관계가 그대로 적용된다.
...
- `template.ftl`은 [CH18. Theme 커스터마이징](/study/keycloak/18-theme)에서 다룰 공통 레이아웃이다.
...
ProviderConfigProperty는 setter 체인과 5-arg 생성자(name, label, helpText, type, defaultValue) 모두 지원한다. 이 챕터는 가독성 위주로 setter 패턴을 사용하며, 생성자 방식은 섹션 후반 예시에 나온다.
...
[CH10. Authentication Flow](/study/keycloak/10-auth-flow)에서 다룬 Flow 구조에 새 Authenticator를 끼워 넣어야 실제로 동작한다.
...
public class EmployeeAuthenticator implements Authenticator {
    ...
    public void action(AuthenticationFlowContext context) {
        ...
        AuthenticatorConfigModel config = context.getAuthenticatorConfig();
        // AuthenticatorConfig는 Admin Console에서 설정됐다고 가정 (null 체크는 ConfigurableAuthenticatorFactory의 default properties로 보장)
...
- Flow 배치는 [CH10](/study/keycloak/10-auth-flow)의 Required/Alternative 규칙을 따르며 Conditional 서브플로우로 조건부 적용이 가능하다.
:::

## 다음 챕터

- 이전 : [SPI로 Keycloak 확장](/study/keycloak/15-spi-overview)
- 다음 : [커스텀 User Storage](/study/keycloak/17-custom-user-storage)
