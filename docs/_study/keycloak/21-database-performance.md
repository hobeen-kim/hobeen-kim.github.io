...
prev: /study/keycloak/20-k8s-operator
next: /study/keycloak/22-admin-rest-api
---
...
| 테이블 | 역할 | 주요 컬럼 | 관련 챕터 |
|--------|------|----------|----------|
| `REALM` | Realm 메타데이터 | ID, NAME, ENABLED | [CH3](/study/keycloak/03-realm-organizations) |
| `CLIENT` | Client 정의 | ID, CLIENT_ID, REALM_ID | [CH4](/study/keycloak/04-client-service-account) |
| `USER_ENTITY` | 사용자 기본 정보 | ID, USERNAME, EMAIL, REALM_ID | [CH5](/study/keycloak/05-user-credentials) |
| `CREDENTIAL` | 비밀번호/OTP/WebAuthn | USER_ID, TYPE, CREDENTIAL_DATA | [CH5](/study/keycloak/05-user-credentials) |
| `USER_ROLE_MAPPING` | 사용자-역할 M:N | USER_ID, ROLE_ID | [CH6](/study/keycloak/06-role-group) |
| `USER_ATTRIBUTE` | 사용자 커스텀 속성 | USER_ID, NAME, VALUE | [CH5](/study/keycloak/05-user-credentials) |
| `KEYCLOAK_ROLE` | 역할(Realm/Client 공용) | ID, NAME, CLIENT_ROLE | [CH6](/study/keycloak/06-role-group) |
...
| `FEDERATED_IDENTITY` | 외부 IdP 매핑 | USER_ID, IDENTITY_PROVIDER | [CH14](/study/keycloak/14-identity-brokering) |
...
- `USER_SESSION` (메모리/Infinispan): 현재 로그인 세션 ([CH19](/study/keycloak/19-ha-clustering))
...
환경변수로는 `KC_DB_POOL_INITIAL_SIZE`, `KC_DB_POOL_MIN_SIZE`, `KC_DB_POOL_MAX_SIZE`다. Operator에서는 [CH20](/study/keycloak/20-k8s-operator)에서 본 `spec.db.poolMaxSize` 필드가 직접 매핑된다.
...
Agroal은 Prometheus 메트릭을 노출한다([CH24](/study/keycloak/24-monitoring-upgrade)).
...
[CH19](/study/keycloak/19-ha-clustering)에서 본 캐시 5종 중 유일하게 DB에 영속되는 것이 <strong>오프라인 세션</strong>이다. Refresh Token이 발급된 사용자 정보가 `OFFLINE_USER_SESSION`에 저장된다.
...
Persistent user sessions 활성화 플래그는 `--features=persistent-user-sessions`이며 v26 마이너 버전에 따라 기본 활성화 여부가 다르다. 사용 중인 버전 릴리스 노트로 확인한다.

### 오프라인 세션 정리

오프라인 세션은 자동 정리되지 않는 경우가 있다. Revoke 요청이 들어왔거나, 세션 유효기간이 지나도 `OFFLINE_USER_SESSION` 행이 남아 있을 수 있다. 주기적인 모니터링이 필요하다.

Keycloak의 PostgreSQL 실제 테이블명은 대문자(예: `OFFLINE_USER_SESSION`)이지만, PostgreSQL은 따옴표 없이 쓰면 소문자로 접히므로 아래 SQL은 큰따옴표 없이 소문자로도 동작한다. 운영 환경에서는 실제 스키마 확인 후 사용한다.
...
아래 인덱스 이름은 Keycloak 기본 Liquibase 마이그레이션 기준이다. 버전·배포별로 이름이 다를 수 있으므로 실제 환경에서는 `\di` 또는 카탈로그 쿼리로 확인한다.
...
| 1000만~ | 커스텀 UserStorage SPI로 외부 DB 연동([CH17](/study/keycloak/17-custom-user-storage)) |

### 100만 이상에서 생기는 문제

- <strong>Admin Console 사용자 목록 페이지가 느려진다</strong>. count 쿼리가 비싸진다. Admin Console 검색 최적화 기능이나 count 비활성화 옵션을 검토한다 (해당 기능 플래그 이름과 지원 여부는 릴리스 노트 확인).
- <strong>대량 사용자 Import가 느리다</strong>. Realm Import로 대량 사용자를 넣으면 한 번에 다 로드된다. 증분 방식으로 REST API 배치 호출이 안전([CH22](/study/keycloak/22-admin-rest-api)).
- <strong>그룹 기반 역할 조회가 무거워진다</strong>. Group을 권한 지급의 주 경로로 쓰면 USER_GROUP_MEMBERSHIP 조인이 자주 발생.

### Realm 분할 전략

단일 Realm에 전체 사용자를 넣지 말고, 조직/지역 기준으로 Realm을 나누는 편이 좋을 때도 있다. [CH3. Realm과 Organizations](/study/keycloak/03-realm-organizations)에서 다룬 멀티테넌시 모델을 참고한다.
...
| 관리자 UI 사용자 목록이 느리다 | Count 쿼리 비용 | count 비활성화 / 검색 최적화 기능 검토 (플래그 이름은 릴리스 노트 확인) |
...
DB와 캐시 튜닝으로 Keycloak을 견고하게 했다면, 이제 그 위에서 수행하는 운영 작업을 자동화할 차례다. [CH22. Admin REST API와 자동화](/study/keycloak/22-admin-rest-api)에서 Admin REST API, Service Account 인증, Terraform Provider로 IAM 상태를 선언적으로 관리하는 방법을 다룬다.

- 이전: [CH20. Kubernetes + Operator 배포](/study/keycloak/20-k8s-operator)
- 다음: [CH22. Admin REST API와 자동화](/study/keycloak/22-admin-rest-api)
