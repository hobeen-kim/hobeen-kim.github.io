...
SAE(Society of Automotive Engineers)가 정의한 **대형 차량 및 작업 기계용 통신 상위 프로토콜**이다.
...
Classic CAN 2.0B는 29비트 확장 ID를 제공하지만, 이 ID를 어떻게 쓸지는 규정하지 않는다. J1939은 이 29비트를 다음과 같이 **명확한 필드로 구조화**한다.
...
J1939은 노드마다 **고유한 소스 어드레스(SA)**를 부여한다. SAE J1939-81(네트워크 관리) 규격이 주소 요청/충돌 감지 절차를 정의한다.
...
- J1939은 CAN 2.0B의 **29비트 ID**를 Priority + EDP + DP + PF + PS + SA 필드로 구조화한다.
- **SA(Source Address)**로 버스 내 노드를 구분하며, 0x00~0xFD가 일반 노드에 할당된다.
- ISOBUS는 J1939을 농기계 도메인에 맞게 확장한 프로토콜이다.
:::

---

[이전: CAN FD](/study/isobus/07-can-fd) | [다음: J1939 메시지 구조](/study/isobus/09-j1939-message)
