...
prev: /study/keycloak/18-theme
next: /study/keycloak/20-k8s-operator
---
...
클러스터 기본 구조를 이해했다면 이제 실제 배포 형태를 다룬다. [CH20. Kubernetes + Operator 배포](/study/keycloak/20-k8s-operator)에서 Keycloak Operator로 프로덕션급 Keycloak을 쿠버네티스에 올리는 방법과, Realm을 GitOps로 선언 관리하는 패턴을 살펴본다.

- 이전: [CH18. Theme 커스터마이징](/study/keycloak/18-theme)
- 다음: [CH20. Kubernetes + Operator 배포](/study/keycloak/20-k8s-operator)
