...
CAN은 **두 개의 선(CAN_H, CAN_L)**으로 신호를 전달합니다. 이 방식을 **차동 신호(Differential Signal)**라고 합니다.

### 왜 두 선이 필요한가

한 선만 쓰면(단선 신호) 외부 노이즈가 신호에 직접 더해집니다. 그런데 두 선을 쓰면 노이즈가 두 선에 **같은 크기로** 더해집니다. 수신 측은 두 선의 **전압 차이**만 보기 때문에, 두 선에 동일하게 더해진 노이즈는 자동으로 상쇄됩니다.
...
버스에 연결된 노드 중 **단 하나라도 Dominant(0)**를 보내면, 버스 전체가 Dominant가 됩니다. 모든 노드가 Recessive(1)를 보낼 때만 버스가 Recessive 상태가 됩니다.
...
**종단 저항(Termination Resistor)**은 버스 양 끝에 달아 신호를 흡수해 반사를 막습니다. 저항값이 선로 특성 임피던스(120Ω)와 같아야 합니다.
...
두 종단 저항이 병렬 연결되므로 버스 전체 임피던스는 **60Ω**입니다. 이 값이 맞지 않으면 파형이 불안정해집니다.
...
트랙터와 작업기 연결에는 **AMP Deutsch DT 시리즈** 방수 커넥터가 많이 사용됩니다. 진단 목적으로는 **9핀 Deutsch 커넥터**가 표준입니다.
...
::: tip 핵심 정리
- CAN은 CAN_H/CAN_L 두 선의 **전압 차이**로 신호를 전달해 노이즈에 강합니다.
