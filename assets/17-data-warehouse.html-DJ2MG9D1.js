...
</pre><h3 id="snowflake-스키마" tabindex="-1"><a class="header-anchor" href="#snowflake-스키마"><span>Snowflake 스키마</span></a></h3><p>Star 스키마에서 디멘션 테이블을 추가로 정규화한 구조이다. 예를 들어 <code>product_dim</code>의 <code>category</code>를 별도 <code>category_dim</code>으로 분리한다. 데이터 중복은 줄어들지만 조인이 많아져 쿼리가 복잡해진다.</p><table><thead><tr><th>비교</th><th>Star 스키마</th><th>Snowflake 스키마</th></tr></thead><tbody><tr><td>정규화 수준</td><td>낮음 (디멘션 비정규화)</td><td>높음 (디멘션 정규화)</td></tr><tr><td>쿼리 복잡도</td><td>낮음</td><td>높음 (다단계 조인)</td></tr><tr><td>저장 공간</td><td>많이 사용</td><td>절약</td></tr><tr><td>쿼리 성능</td><td>빠름</td><td>상대적으로 느림</td></tr></tbody></table><p>실무에서는 대부분 Star 스키마를 선호한다.</p><h2 id="_3-etl-extract-transform-load" tabindex="-1"><a class="header-anchor" href="#_3-etl-extract-transform-load"><span>3. ETL (Extract-Transform-Load)</span></a></h2><h3 id="etl-개념" tabindex="-1"><a class="header-anchor" href="#etl-개념"><span>ETL 개념</span></a></h3><p><strong>ETL</strong>은 운영 시스템(OLTP DB, 로그, 외부 API 등)에서 데이터를 추출하고, 분석에 적합한 형태로 변환하며, 데이터 웨어하우스에 적재하는 과정이다.</p><p><strong>Extract (추출)</strong> — 소스 시스템에서 데이터를 읽는다. 전체 추출(Full Extract) 또는 변경분만 추출(Incremental Extract)을 사용한다.</p><p><strong>Transform (변환)</strong> — 데이터를 정제, 통합, 집계한다. 중복 제거, 결측값 처리, 코드 변환(예: &#39;M&#39; → &#39;남성&#39;), 여러 소스 통합, 집계 계산 등이 포함된다.</p><p><strong>Load (적재)</strong> — 변환된 데이터를 DW에 저장한다. 전체 교체(Full Load) 또는 증분 추가(Incremental Load) 방식이 있다.</p><h3 id="etl-파이프라인" tabindex="-1"><a class="header-anchor" href="#etl-파이프라인"><span>ETL 파이프라인</span></a></h3><pre class="mermaid">flowchart LR
...
<span class="line"></span></code></pre><div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0;"><div class="line-number"></div></div></div><h2 id="_4-olap-연산" tabindex="-1"><a class="header-anchor" href="#_4-olap-연산"><span>4. OLAP 연산</span></a></h2><p>OLAP에서는 다차원 데이터 큐브를 다양한 각도로 분석하는 연산을 사용한다. 온라인 쇼핑몰의 매출 데이터(상품, 지역, 시간 차원)를 예로 든다.</p><h3 id="roll-up-합산-상위-집계" tabindex="-1"><a class="header-anchor" href="#roll-up-합산-상위-집계"><span>Roll-up (합산/상위 집계)</span></a></h3><p>데이터를 상위 계층으로 집계한다. 세부 수준에서 요약 수준으로 올라간다.</p><div class="language-text line-numbers-mode" data-highlighter="prismjs" data-ext="text" data-title="text"><pre><code><span class="line">일별 매출 → 월별 매출 → 분기별 매출 → 연별 매출</span>
