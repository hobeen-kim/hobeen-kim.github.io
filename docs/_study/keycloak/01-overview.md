...
tags: [Keycloak, IAM]
prev: /study/keycloak/
next: /study/keycloak/02-core-concepts
---
...
| Red Hat Single Sign-On (RH-SSO) | 구 제품명, RHBK로 대체됨 | RHBK로 대체된 구 상용 브랜드 |
...
```mermaid
flowchart LR
    OAuth[OAuth 스터디<br>CH17. Keycloak] -->|"Keycloak으로<br>AS 구축 Quick Tour"| KC1[Keycloak 스터디<br>CH1~2. 개요·용어]
    KC1 --> KC2[CH3~9. 객체 모델·토큰·권한]
    KC2 --> KC3[CH10~14. 인증 정책·페더레이션]
    KC3 --> KC4[CH15~18. SPI 확장]
    KC4 --> KC5[CH19~24. 운영]
```

### 이 스터디를 읽는 순서 제안

- 이미 Keycloak을 운영 중이라면 CH1~2를 훑고 CH3부터 정독.
- 신규 도입이라면 CH2(개념·용어) → CH3(Realm)부터 순서대로 읽고 Admin Console 화면을 눈으로 함께 따라가는 것을 추천.
- SPI 커스터마이징만 관심이라면 CH4·CH10·CH15 이후를 중점 학습.
- Kubernetes 배포만 관심이라면 CH19~20를 먼저 읽고 필요할 때 앞장으로 돌아오기.

### 이 스터디의 범위 한눈에

24개 챕터 전체가 다룰 주제를 범주별로 정리하면 다음과 같다. 챕터 사이의 연결은 서두 목차의 Mermaid 로드맵에 그려 뒀다.

| 범주 | 챕터 | 무엇을 배우는가 |
|------|------|----------------|
| 개요 | CH1 | Keycloak 개요·라이선스 |
| 개념·용어 | CH2 | 핵심 객체 모델과 용어 사전 |
| 객체 모델 | CH3~6 | Realm·Client·User·Role |
| 토큰·권한·프로토콜 | CH7~9 | Scope·Mapper·UMA·SAML·Token Exchange |
| 인증 정책 | CH10~12 | Flow·Policy·MFA |
| 페더레이션 | CH13~14 | LDAP·Social IdP |
| SPI 확장 | CH15~18 | Authenticator·User Storage·Theme |
| 운영 | CH19~24 | HA·K8s·DB·REST API·Backup·Upgrade |
...
- kubectl + minikube/kind (CH20~21 K8s 실습 시)

준비가 부족해도 CH1~14은 Docker Compose만으로 충분히 따라올 수 있다. K8s·SPI 실습은 해당 챕터 도달 시점에 추가 설치를 검토하면 된다.
...
- "토큰 발급 속도가 충분한가" → Quarkus 전환 후 단일 Pod에서 수백 TPS 이상 가볍게 처리한다. Infinispan 캐시(CH19)와 DB 튜닝(CH21)을 병행하면 대규모 트래픽도 문제없다.
- "업그레이드가 쉬운가" → 쉬운 편은 아니다. 메이저 버전마다 Breaking Change가 누적된다. 그래서 이 스터디의 마지막 챕터(CH24)가 업그레이드와 모니터링에 통째로 할당돼 있다.
...
- Docker 이미지 태그는 집필 시점 기준 `quay.io/keycloak/keycloak:26.1`을 사용한다. 최신 마이너 버전은 [Keycloak Releases](https://www.keycloak.org/docs/latest/release_notes/)에서 확인한다.
- Admin Console 스크린 설명은 v26 레이아웃 기준이다. v24 이하는 메뉴 구조가 일부 다르다.
- Quarkus 배포판만 다룬다. Wildfly 배포판은 Keycloak 20에서 완전히 제거된 구 스택이다.
...
- 이 스터디는 OAuth CH17의 소비자 관점을 넘어 제공자·운영자 관점을 24챕터에 걸쳐 다룬다.
:::

## 다음 챕터

- 이전 : [Keycloak 스터디 개요](/study/keycloak/)
- 다음 : [핵심 개념과 용어](/study/keycloak/02-core-concepts)
