...
**Virtual Terminal(VT)**은 ISO 11783-6에 정의된 표준 사용자 인터페이스 시스템입니다. 트랙터 운전석에 장착된 디스플레이에 **작업기의 UI를 표시하고 사용자 입력을 받는** 인터페이스 역할을 합니다.
...
즉, **디스플레이 하드웨어는 트랙터가 제공**하고, **화면에 무엇을 보여줄지는 작업기 ECU가 결정**합니다. 트랙터와 작업기의 역할이 명확히 분리되어 있는 것이 핵심입니다.
...
VT 표준이 없던 시절, 각 작업기 제조사는 **전용 디스플레이 패널**을 함께 납품했습니다.
...
VT는 이 문제를 **스마트폰 앱 스토어 모델**로 해결했습니다.
...
하나의 VT 디스플레이에 어떤 제조사의 작업기를 연결해도, 작업기가 자신의 UI 정의를 VT로 전송하면 VT가 화면을 렌더링합니다. 운전자는 **하나의 디스플레이로 모든 작업기를 조작**할 수 있습니다.
...
ISO 11783-6은 지속적으로 개정되어 왔으며, VT Server와 Client가 지원하는 **버전(Version)**에 따라 사용 가능한 기능이 달라집니다.
...
VT Server와 Client가 서로 다른 버전을 지원할 경우, **낮은 버전으로 협상**하여 동작합니다. 협상 과정은 VT Status 메시지 교환 시 이루어집니다.
...
**AUX(Auxiliary Input)**는 조이스틱, 추가 버튼 패드, 풋 페달 등 VT 디스플레이 외부의 보조 입력 장치를 ISOBUS에 통합하는 메커니즘입니다. ISO 11783-6 Annex에 정의되어 있습니다.
...
AUX의 핵심 개념 중 하나는 **Preferred Assignment**입니다. 사용자가 한 번 입력 장치와 기능의 매핑을 설정하면, 그 설정이 VT에 저장됩니다. 다음에 같은 작업기를 연결하면 **이전 매핑이 자동으로 복원**됩니다.
