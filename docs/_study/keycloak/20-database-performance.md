...
prev: /study/keycloak/19-k8s-operator
next: /study/keycloak/21-admin-rest-api
---
...
| 테이블 | 역할 | 주요 컬럼 | 관련 챕터 |
|--------|------|----------|----------|
| `REALM` | Realm 메타데이터 | ID, NAME, ENABLED | [CH2](/study/keycloak/02-realm-organizations) |
| `CLIENT` | Client 정의 | ID, CLIENT_ID, REALM_ID | [CH3](/study/keycloak/03-client-service-account) |
| `USER_ENTITY` | 사용자 기본 정보 | ID, USERNAME, EMAIL, REALM_ID | [CH4](/study/keycloak/04-user-credentials) |
| `CREDENTIAL` | 비밀번호/OTP/WebAuthn | USER_ID, TYPE, CREDENTIAL_DATA | [CH4](/study/keycloak/04-user-credentials) |
| `USER_ROLE_MAPPING` | 사용자-역할 M:N | USER_ID, ROLE_ID | [CH5](/study/keycloak/05-role-group) |
| `USER_ATTRIBUTE` | 사용자 커스텀 속성 | USER_ID, NAME, VALUE | [CH4](/study/keycloak/04-user-credentials) |
| `KEYCLOAK_ROLE` | 역할(Realm/Client 공용) | ID, NAME, CLIENT_ROLE | [CH5](/study/keycloak/05-role-group) |
...
| `FEDERATED_IDENTITY` | 외부 IdP 매핑 | USER_ID, IDENTITY_PROVIDER | [CH13](/study/keycloak/13-identity-brokering) |
...
- `USER_SESSION` (메모리/Infinispan): 현재 로그인 세션 ([CH18](/study/keycloak/18-ha-clustering))
...
환경변수로는 `KC_DB_POOL_INITIAL_SIZE`, `KC_DB_POOL_MIN_SIZE`, `KC_DB_POOL_MAX_SIZE`다. Operator에서는 [CH19](/study/keycloak/19-k8s-operator)에서 본 `spec.db.poolMaxSize` 필드가 직접 매핑된다.
...
Agroal은 Prometheus 메트릭을 노출한다([CH23](/study/keycloak/23-monitoring-upgrade)).
...
[CH18](/study/keycloak/18-ha-clustering)에서 본 캐시 5종 중 유일하게 DB에 영속되는 것이 <strong>오프라인 세션</strong>이다. Refresh Token이 발급된 사용자 정보가 `OFFLINE_USER_SESSION`에 저장된다.
...
| 1000만~ | 커스텀 UserStorage SPI로 외부 DB 연동([CH16](/study/keycloak/16-custom-user-storage)) |
...
- <strong>대량 사용자 Import가 느리다</strong>. Realm Import로 대량 사용자를 넣으면 한 번에 다 로드된다. 증분 방식으로 REST API 배치 호출이 안전([CH21](/study/keycloak/21-admin-rest-api)).
- <strong>그룹 기반 역할 조회가 무거워진다</strong>. Group을 권한 지급의 주 경로로 쓰면 USER_GROUP_MEMBERSHIP 조인이 자주 발생.

### Realm 분할 전략

단일 Realm에 전체 사용자를 넣지 말고, 조직/지역 기준으로 Realm을 나누는 편이 좋을 때도 있다. [CH2. Realm과 Organizations](/study/keycloak/02-realm-organizations)에서 다룬 멀티테넌시 모델을 참고한다.
...
DB와 캐시 튜닝으로 Keycloak을 견고하게 했다면, 이제 그 위에서 수행하는 운영 작업을 자동화할 차례다. [CH21. Admin REST API와 자동화](/study/keycloak/21-admin-rest-api)에서 Admin REST API, Service Account 인증, Terraform Provider로 IAM 상태를 선언적으로 관리하는 방법을 다룬다.

- 이전: [CH19. Kubernetes + Operator 배포](/study/keycloak/19-k8s-operator)
- 다음: [CH21. Admin REST API와 자동화](/study/keycloak/21-admin-rest-api)
