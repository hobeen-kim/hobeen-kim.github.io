...
기존 tuple을 신규 relation으로 복사한다. `ExportBulkRelationships` → 필터링 → `ImportBulkRelationships` 조합이나 Watch API 기반 실시간 마이그레이터를 쓴다. 동시에 애플리케이션의 쓰기 경로는 "양쪽 모두에 기록"(dual write)하도록 배포한다. 읽기는 아직 기존 쪽. 이 단계가 가장 길고, 며칠~몇 주 걸린다.
...
for {
    resp, _ := spicedb.ExportBulkRelationships(ctx, &pb.ExportBulkRelationshipsRequest{
        RelationshipFilter: &pb.RelationshipFilter{
            ResourceType:     "document",
            OptionalRelation: "viewer",
        },
        OptionalCursor: &pb.Cursor{Token: cursorToken},
    })
    ...
    spicedb.ImportBulkRelationships(ctx, &pb.ImportBulkRelationshipsRequest{
        Relationships: newRels,
    })
    if resp.AfterResultCursor == nil || resp.AfterResultCursor.Token == "" { break }
    cursorToken = resp.AfterResultCursor.Token
}
```

::: info RPC 형태 주의
`ExportBulkRelationships`는 서버 스트리밍 RPC, `ImportBulkRelationships`는 클라이언트 스트리밍 RPC로 구현된 SpiceDB 버전이 많다. 실제 Go 클라이언트에서는 `stream, _ := client.ExportBulkRelationships(ctx, req)` 후 `stream.Recv()` 루프로 받고, Import도 `stream.Send(...)` → `CloseAndRecv()` 흐름을 쓴다. 위 의사코드는 논리 흐름만 압축해 보여준 것이며, 배포 버전의 proto·클라이언트 문서를 기준으로 최종 형태를 확정한다.
:::
...
sequenceDiagram
    ...
    W->>S: ExportBulkRelationships(viewer)
    W->>S: ImportBulkRelationships(reader)
...
해결책은 tuple을 모두 재생성하는 것이다. 기존 `viewer` tuple을 `ExportBulkRelationships`로 꺼내 Caveat context를 붙여 `ImportBulkRelationships`로 다시 넣는다. 초기 context는 보수적으로 "모든 IP 허용" 같은 기본값을 주고, 이후 실제 정책을 점진 반영한다. 대규모 tuple에서는 이 과정이 며칠이 걸리므로 반드시 점진 배포다.
...
Git에 schema를 두면 코드 리뷰 문화를 그대로 적용할 수 있다. PR에는 "이 변경이 Additive인가 Destructive인가", "Expand/Migrate/Contract 중 어느 단계인가"를 템플릿으로 강제한다. CI는 `zed validate validation.yaml`로 schema·assertion·expected relations를 한 번에 검증해 회귀를 막는다. validation YAML 상단의 `schema:` 블록에 `.zed` schema 본문을 그대로 포함시키는 것이 표준 구성이다.
