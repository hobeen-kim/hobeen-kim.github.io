...
prev: /study/keycloak/08-saml-token-exchange
next: /study/keycloak/10-password-policy
---
...
Authentication SPI는 Keycloak의 확장 포인트 중 가장 활발한 영역이다(CH14~17에서 다룬다). 엔진은 다음을 담당한다.
...
Keycloak 기본에는 IP 기반 Condition이 없으므로 실무에선 SPI로 `IpCheckAuthenticator`를 만들어 `providers/` 디렉토리에 배치한다([CH15. 커스텀 Authenticator](/study/keycloak/15-custom-authenticator) 예고).
...
- **Git 관리**: Realm Import/Export JSON에 Flow 정의가 포함되므로, 변경 사항을 [CH22. Backup/Restore](/study/keycloak/22-backup-restore)에서 다룰 Export로 관리한다.
...
- **변경 직후 모니터링** — [CH23. 모니터링](/study/keycloak/23-monitoring-upgrade)에서 다룰 Event Listener로 `LOGIN_ERROR` 급증을 감지.

### Realm Export와 Flow

Flow는 Realm 정의 JSON에 `authenticationFlows` 배열로 포함된다. GitOps로 관리하려면 Export 스냅샷을 기준으로 PR 리뷰를 하고, [CH21. Admin REST API](/study/keycloak/21-admin-rest-api)로 일관성 있게 적용한다.
...
- 이전 : [SAML 2.0과 Token Exchange](/study/keycloak/08-saml-token-exchange)
- 다음 : [Password Policy와 Brute Force](/study/keycloak/10-password-policy)
