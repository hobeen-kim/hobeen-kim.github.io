...
- **데이터 프레임**: "본부, 현재 위치 좌표는 37.5°N, 127.0°E입니다." — 실제 정보 전달
- **리모트 프레임**: "본부, 현재 온도 데이터 좀 보내줘요." — 데이터 요청
- **에러 프레임**: "잠깐! 방금 전송에 오류가 있었습니다!" — 오류 신호
- **오버로드 프레임**: "저 지금 처리 중이에요, 잠깐만요." — 전송 지연 요청
...
> [!TIP]
> **Dominant vs Recessive**: CAN 버스에서 0을 Dominant, 1을 Recessive라 부른다. 여러 노드가 동시에 신호를 내보낼 때 Dominant(0)가 항상 이긴다. 전선을 당기는(0) 쪽이 안 당기는(1) 쪽을 이기는 것과 같다.
...
같은 값의 비트가 **5개 연속** 나타나면, 다음에 반드시 **반대 값의 비트를 1개 삽입**한다. 이 삽입된 비트를 **Stuff Bit**이라 한다.
...
- CAN 프레임에는 데이터/리모트/에러/오버로드 4가지 종류가 있으며, 가장 많이 쓰이는 것은 **데이터 프레임**이다.
...
- **비트 스터핑**은 5개 연속 동일 비트 뒤에 반대 비트를 삽입해 수신 노드의 동기화를 돕는다.
...
- **이전**: [03. CAN 물리 계층](/study/isobus/03-can-physical)
- **다음**: [05. CAN 중재와 우선순위](/study/isobus/05-can-arbitration)
