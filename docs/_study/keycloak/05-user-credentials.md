...
prev: /study/keycloak/04-client-service-account
next: /study/keycloak/06-role-group
---
...
| `recovery-authn-codes` | 일회용 복구 코드 배열 | 카운터 매칭, MFA 분실 대비 (v22+) |
| `secret-questions` | 질문/답 해시 | 예제 Extension으로만 제공. 기본 내장 아님. 저항력 약함 |
...
다음은 개념적 예시다. 실제 DB의 `CREDENTIAL.secret_data` 컬럼은 Keycloak 구현 세부사항에 따라 `additionalParameters` 맵 등으로 다르게 저장될 수 있으니 마이그레이션·디버깅 시 반드시 실제 스키마를 확인한다.
...
레거시 시스템에서 BCrypt로 저장된 해시를 그대로 Keycloak으로 옮기려면 별도 Credential Provider SPI가 필요하다(CH15~18). 단순 이관이 아니라 "레거시 해시를 검증만 하다가, 로그인 성공 시 Keycloak 표준 해시로 재해싱"하는 패턴이 실무 표준이다.
...
기본값 `HmacSHA1`은 RFC 6238 기본이고, 대부분의 Authenticator 앱과 호환성 유지를 위해 실무에서도 기본값을 그대로 둔다. HmacSHA256/512는 앱 호환성이 제한되므로 이전에 신중히 검토한다.
...
- Recovery Codes 기능 활성화(CH12에서 상세).
...
사용자가 스스로 자격 증명을 관리하려면 Account Console (`{KC_HOSTNAME}/realms/{realm}/account/`)을 통한다. 여기서 Password, OTP, WebAuthn을 자발적으로 추가/삭제할 수 있다. Admin Console의 Credentials 탭과 다루는 대상은 같지만, 사용자 본인이 운영 주체라는 점이 다르다.
...
- 이전 : [Client와 Service Account](/study/keycloak/04-client-service-account)
- 다음 : [Role·Group과 Composite Role](/study/keycloak/06-role-group)
