...
Script Mapper는 유연하지만 기본적으로 비활성화된 Preview 기능이다. 활성화하려면 서버 시작 시 `--features=scripts` 플래그를 켜야 하고, 보안상 JS 실행 환경(Nashorn→Rhino 등)의 제약이 있다. 운영 환경에서는 [CH15 SPI](/study/keycloak/15-spi-overview)에서 다룰 커스텀 Mapper Provider로 대체하는 것이 일반적이다.
