...
| Look Ahead Window | 1 | 시계 표류 허용 윈도(미래 방향) |
...
Look Ahead Window = N이면 서버는 현재 및 이후 N개 시간 창(각 30초)의 OTP를 허용. 사용자 시계가 느릴 때 수용하는 옵션이며, 과거 방향 코드는 허용하지 않는다. 너무 크게 두면 공격 창이 넓어지므로 1~2 권장.
...
<strong>Passkey</strong>는 Resident Credential의 브랜딩 용어다. Synced Passkey(iCloud/Google 계정 동기화)와 Device-bound Passkey(YubiKey 같은 보안키에 묶인 형태) 모두를 포함한다. 사용자 입장에서는 "이 아이폰에서 등록한 로그인 방식이 맥북에서도 그대로 쓰인다"가 된다(Synced 형태). Keycloak은 WebAuthn 정책의 <strong>Require Resident Key</strong>를 `Yes`(WebAuthn 명세 기준 `required`)로 설정해 Passkey 방식을 강제할 수 있다.
...
| Require Resident Key | `Not specified`/`Yes`/`No` (WebAuthn 명세 기준 `discouraged`/`required`/`preferred`) | Passkey 강제 여부 |
...
Client가 `acr_values=2` 또는 OIDC `claims` 파라미터로 요청한다. Keycloak은 Realm Settings → Authentication의 ACR-to-LoA(Level of Authentication) 매핑을 정적으로 두고, 인증 Flow의 Conditional Subflow 안에 `Condition - Level of Authentication` Authenticator를 배치해 요청된 LoA에 도달하도록 Subflow 진입 여부를 결정한다. Execution이 런타임에 "끼워지는" 것이 아니라 Flow 구성이 정적이라는 점이 핵심이다. 고위험 기능 화면에서만 MFA를 요구하고 싶을 때 쓴다.
...
- **OTP 시계 표류 재발**: NTP 동기화 점검, Look Ahead Window 임시 확대 후 원인 해결 시 원복.

::: tip 핵심 정리
- OTP Policy는 TOTP/HOTP 선택·해시 알고리즘·자리수·Look Ahead Window를 조절하며 Google Authenticator 호환을 위해 SHA-1이 기본이고 고위험 환경은 SHA-256·8자리를 고려한다.
