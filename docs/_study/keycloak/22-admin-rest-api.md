...
prev: /study/keycloak/21-database-performance
next: /study/keycloak/23-backup-restore
---
...
토큰 수명은 master realm 기본값이 1분(Realm 설정에 따라 다르므로 운영 환경에 맞춰 조정). 스크립트가 오래 걸리면 중간에 재발급한다.
...
| DELETE | `/admin/realms/{realm}/sessions/{session}` | 세션 강제 종료 |

사용자별 로그아웃은 `POST /admin/realms/{realm}/users/{userId}/logout`도 있다.

Event는 [CH24. 모니터링·감사와 업그레이드](/study/keycloak/24-monitoring-upgrade)에서 자세히 다룬다.
...
- count는 별도 엔드포인트(`/users/count`)로 호출한다. [CH21](/study/keycloak/21-database-performance)에서 본 "count가 느려지는" 문제와 연결된다.
...
REST API 호출은 명령형이다. 선언적으로 상태를 관리하려면 <strong>Terraform keycloak Provider</strong>가 표준 선택지다.

구 `mrparkers/keycloak` Provider는 2023년 아카이브됐다. 현재는 `keycloak/keycloak`(또는 fork된 active 유지 버전)을 쓰며, 사용 전 [Terraform Registry](https://registry.terraform.io/)에서 active maintained 여부를 확인한다.
...
[CH20](/study/keycloak/20-k8s-operator)에서 본 `KeycloakRealmImport`와 Terraform이 겹쳐 보일 수 있다. 실전에서는 보통 역할을 분리한다.
...
- Admin Event: [CH24](/study/keycloak/24-monitoring-upgrade)의 Admin Events를 SIEM으로 전달해 누가·무엇을·언제 바꿨는지 추적.
...
자동화로 Realm 상태를 일관되게 관리하는 방법을 배웠다. 이제 그 상태를 안전하게 백업·복구하는 영역으로 넘어간다. [CH23. Backup/Restore와 Realm 이관](/study/keycloak/23-backup-restore)에서 Realm JSON Export/Import와 DB 백업의 차이, 재해 복구 관점의 RTO/RPO 전략을 다룬다.

- 이전: [CH21. 데이터베이스와 성능](/study/keycloak/21-database-performance)
- 다음: [CH23. Backup/Restore와 Realm 이관](/study/keycloak/23-backup-restore)
