...
description: "정밀 농업의 핵심 컴포넌트인 Task Controller의 개념, 역할, Section/Rate Control, GPS 연동을 이해합니다."
date: 2026-04-13
...
TC는 두 가지 핵심 기능을 수행합니다.

- **자동 제어**: 작업 계획(처방 맵, Prescription Map)에 따라 작업기를 자동으로 제어합니다. 밭의 위치별로 미리 지정된 살포량·파종량을 GPS 위치와 연동하여 자동으로 적용합니다.
- **작업 기록**: 실제 작업 결과(As-Applied Data)를 수집하고 기록합니다. 어느 위치에서 얼마나 살포했는지를 나중에 분석할 수 있도록 로그로 남깁니다.

TC가 없던 시대에는 농민이 수동으로 살포량을 조절해야 했습니다. TC는 이 과정을 자동화하여 비료·농약 과용을 줄이고 생산성을 높입니다.

---

## 2. TC의 역할

TC는 FMIS(Farm Management Information System, 농장관리시스템)와 작업기 사이를 연결하는 중간 다리입니다.
...
ISOBUS에서 TC는 역할에 따라 두 가지로 구분됩니다.
...
GPS 위치에 따라 살포량(Rate)을 **가변적으로** 제어하는 기능입니다. 처방 맵에 지정된 위치별 목표량을 실시간으로 반영합니다.
...
두 기능은 함께 사용할 수 있습니다. 예를 들어, 밭의 경계에서 외부 구획은 OFF(Section Control)하면서 내부 구획의 살포량은 위치별로 조절(Rate Control)할 수 있습니다.

---

## 5. GPS 연동과 위치 기반 제어

TC는 GPS 위치 데이터를 이용해 현재 위치에 해당하는 처방 맵의 값을 조회합니다.

### 위치 관련 PGN

ISOBUS에서 GPS 정보는 다음 PGN으로 전달됩니다.
...
처방 맵은 일반적으로 격자(Grid) 또는 폴리곤(Polygon) 형태로 밭을 구획하고, 각 구획에 목표 값을 지정해 놓습니다. TC는 현재 GPS 위치가 어느 구획에 속하는지 계산하고, 해당 구획의 목표 값을 TC-Client에 전달합니다.

### NMEA와 ISOBUS

GPS 수신기는 NMEA 0183(시리얼) 또는 NMEA 2000(CAN 기반) 프로토콜로 데이터를 제공합니다. ISOBUS 네트워크에서는 NMEA 2000과 ISOBUS가 같은 CAN 버스를 공유하거나, 브리지를 통해 연결될 수 있습니다.
...
이전: [VT 명령어](/study/isobus/17-vt-commands) | 다음: [TC 프로세스 데이터](/study/isobus/19-tc-process-data)
