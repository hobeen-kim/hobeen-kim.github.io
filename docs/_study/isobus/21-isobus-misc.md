---
title: "ISOBUS 기타 기능"
description: "ISO 11783-14 Sequence Control과 ISO 11783-13 File Server의 동작 원리와 활용 방법을 이해합니다."
date: 2026-04-13
...
두둑 진입 감지는 GPS 경계선 교차, 히치(Hitch) 센서, 또는 운전자 버튼 입력 중 하나로 트리거됩니다.

---

### 1.3 Sequence Control 메시지 구조

Sequence Control은 **SC(Sequence Control)** 노드와 작업기 ECU 사이에서 명령과 상태를 교환합니다.
...
두둑 구간은 TC(Task Controller)의 처방 맵 또는 별도의 Headland Zone 파라미터로 정의됩니다.
...
GPS 좌표가 Headland Zone 경계를 넘는 순간 SC가 자동으로 시퀀스를 실행합니다.
...
File Server는 **Volume → Directory → File** 3단계 계층으로 파일을 관리합니다.
...
클라이언트(TC, VT 등)가 File Server에 접근하는 순서는 다음과 같습니다.
...
아래는 Open File Response를 파싱하는 Python 예시입니다.
