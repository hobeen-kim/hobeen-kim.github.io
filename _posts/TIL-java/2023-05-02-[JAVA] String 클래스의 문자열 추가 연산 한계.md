...
3. `ensureCapacityInternal(count + len);` : 원래 있던 사용량(count) 에 str 의 길이(len) 를 저장공간(value) 에 넣으려고 할 때 저장공간이 부족한지 체크하고 늘려줍니다. 자세히 보겠습니다.
   ...
   - ```java
     ...
     - `Arrays.copyOf(value, newCapacity(minimumCapacity) << coder);` 는 아래와 같습니다.
       ...
       - 만약에 크기가 `MAX_ARRAY_SIZE` 을 넘어간다면 `hugeCapacity(minCapacity)` 를 반환합니다. (이거는 따로 설명하지 않겠습니다. 코드 까보면 재밌습니다. ~~저는 안까봤습니다.~~)
...
3. `sb.append("string2 test")` : 문자열 길이가 12 이므로 총 길이는 23이 되고, 빈 공간이 더 필요하게 됩니다. 한번 빈공간이 더 생기는지 디버깅으로 확인해보겠습니다.
   ...
   - 아래와 같이 count 는 11, str.lengh()  12 인걸 확인할 수 있습니다.
