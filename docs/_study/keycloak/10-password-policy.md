...
prev: /study/keycloak/09-auth-flow
next: /study/keycloak/11-mfa
---
...
PBKDF2-SHA512의 기본값은 버전별로 상향되어 왔다(최근 버전 기준 27,500 → 210,000 등). 반복 횟수를 올릴수록 무차별 대입 비용이 커지는 만큼, 정책 업데이트와 함께 CPU 사용량을 모니터링해야 한다([CH20. DB 성능](/study/keycloak/20-database-performance)과 함께 본다).
...
- 백업에는 해시만 들어 있더라도 유출 시 오프라인 공격이 가능 — [CH22. Backup](/study/keycloak/22-backup-restore)에서 다룰 암호화 저장 필수.
...
CH21에서 다룰 Admin REST API와 결합하면 전체 파이프라인을 코드로 운영할 수 있다. 예: Slack 슬래시 커맨드 → 서비스 계정 토큰 → Keycloak 해제 API 호출 → 감사 로그. 자세한 예제는 [CH21. Admin REST API](/study/keycloak/21-admin-rest-api)로 이어진다.
...
- 이전 : [인증 플로우 커스터마이징](/study/keycloak/09-auth-flow)
- 다음 : [MFA — TOTP / WebAuthn](/study/keycloak/11-mfa)
