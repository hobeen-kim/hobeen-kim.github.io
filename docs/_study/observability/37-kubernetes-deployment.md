...
flowchart TB
    ...
    subgraph Generated["Operator가 생성하는 리소스"]
        CFG["Secret\n(prometheus.yaml)"]
        RULES["ConfigMap/Secret\n(rule 파일)"]
        STS["StatefulSet\n(Prometheus Pod)"]
    end

    PCR -->|watch| OPER
    SM -->|watch, selector 매칭| OPER
    PM -->|watch, selector 매칭| OPER
    PR -->|watch, selector 매칭| OPER
    OPER --> CFG
    OPER --> RULES
    OPER --> STS
    CFG -->|마운트| STS
...
flowchart LR
    subgraph Node1["노드 1"]
        APP1["애플리케이션 Pod"]
        ALLOY1["Alloy\n(DaemonSet)"]
        APP1 -->|"stdout/stderr 로그 파일"| ALLOY1
        APP1 -->|"eBPF profiling"| ALLOY1
    end
    subgraph Node2["노드 2"]
        APP2["애플리케이션 Pod"]
        ALLOY2["Alloy\n(DaemonSet)"]
        APP2 -->|"stdout/stderr 로그 파일"| ALLOY2
        APP2 -->|"eBPF profiling"| ALLOY2
    end

    ALLOY1 -->|"loki.write"| LOKI["Loki"]
    ALLOY2 -->|"loki.write"| LOKI
    ALLOY1 -->|"pyroscope.write"| PYRO["Pyroscope"]
    ALLOY2 -->|"pyroscope.write"| PYRO
```
