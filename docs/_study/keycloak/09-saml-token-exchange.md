...
prev: /study/keycloak/08-authz-uma
next: /study/keycloak/10-auth-flow
---
...
| Valid Redirect URIs | `https://workspace.example.com/*` | SAML 플로우에서 유효 엔드포인트 패턴 검증 |
| Master SAML Processing URL | `https://workspace.example.com/saml/acs` | Assertion Consumer Service(ACS) URL. Response가 POST되는 엔드포인트 |
...
외부 IdP로 처음 로그인한 사용자를 Keycloak 로컬 계정과 어떻게 연결할지는 [CH10. 인증 플로우](/study/keycloak/10-auth-flow)에서 설명할 **First Broker Login** 플로우가 결정한다. 기본 동작은 다음과 같다.

1. 이메일이 기존 사용자와 같으면 "기존 계정에 연결" 선택지를 제시.
2. 없으면 새 사용자로 생성(프로비저닝).

브로커링 설계의 자세한 분류는 [CH14. Identity Brokering](/study/keycloak/14-identity-brokering) 예고.
...
- Token Exchange는 Realm 기능 플래그로 활성화한다. Keycloak 26에서는 Standard Token Exchange v2(`--features=token-exchange-standard-v2`)가 도입됐으며, legacy Token Exchange(`--features=token-exchange`)와 분리되어 권한 모델·파라미터가 달라졌다. 운영 중인 버전의 릴리스 노트로 어느 플래그가 필요한지 먼저 확인한다.
...
주의: `subject_issuer`는 RFC 8693 표준 파라미터가 아니라 Keycloak 확장 파라미터다. 또한 실전 운영 시 `mobile-app` Client의 인증 방식(Service Account + client_secret 또는 private_key_jwt)을 함께 명시해야 한다.
...
- 이전 : [Authorization Services와 UMA 2.0](/study/keycloak/08-authz-uma)
- 다음 : [인증 플로우 커스터마이징](/study/keycloak/10-auth-flow)
