...
개념적 요약 시그니처. 실제 선언은 Keycloak 공식 소스(`UserStorageProviderFactory.java`)를 참조한다.

```java
public interface UserStorageProviderFactory<T extends UserStorageProvider>
        extends ComponentFactory<T, ComponentModel>, ProviderFactory<T> {
    T create(KeycloakSession session, ComponentModel model);
    // ... capabilities, config metadata 등
}
```
...
| 정책 | 동작 | 적합 상황 |
|------|------|----------|
| `DEFAULT` | Realm 전역 `cache` 설정을 그대로 따름 | 기본 정책을 쓸 때 |
