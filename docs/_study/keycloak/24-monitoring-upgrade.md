...
public class SiemEventListenerProvider implements EventListenerProvider {
    ...
    // KafkaProducer.send()는 기본 비동기이지만 acks=all / linger.ms 설정이나 .get() 호출 시 블로킹된다.
    // Event Listener는 로그인 경로에 끼어 있으므로 fire-and-forget 방식(내부 큐·배치)으로 분리한다.
    @Override
...
KafkaProducer.send()는 기본 비동기이지만 `acks=all`·`linger.ms` 설정이나 `.get()` 호출 시 블로킹된다. Event Listener는 로그인 경로에 끼어 있으므로 fire-and-forget 방식(내부 큐·배치)으로 분리한다.

## 3. Metrics

Keycloak v17+는 `--metrics-enabled=true`로 Prometheus 형식 `/metrics` 엔드포인트를 노출한다. <strong>v25+부터 metrics·health 엔드포인트는 별도 management interface(기본 포트 9000)</strong>로 분리됐다. 프로덕션에서는 이 포트를 Prometheus·로드밸런서 헬스체크 경로로 묶고, 외부에는 노출하지 않는다.
...
```bash
curl -s https://auth.example.com:9000/metrics | head -40
```
...
아래 PromQL은 Keycloak v26 Micrometer 지표 이름을 기준으로 한다. 지표·라벨 이름은 버전별로 미세하게 다르므로 실제 `/metrics` 출력과 대조한 뒤 사용한다.

```promql
# 로그인 성공률 (1분 단위)
sum(rate(keycloak_logins_total{provider="keycloak"}[1m]))
  / sum(rate(keycloak_login_attempts_total{provider="keycloak"}[1m]))
...
# DB 풀 포화도
agroal_active_count / agroal_max_used_count  # 실제 지표명은 /metrics 출력에서 확인
...
| Brute Force 탐지 | 사용자 단위 실패율 > 20/min (IP 기반은 `/metrics`에 라벨이 없어 SIEM·WAF에서 처리) | Critical |
...
아직도 Wildfly 기반 구 Keycloak(v19 이하, v20에서 완전 제거)이 프로덕션에 남아 있는 조직이 있다. Quarkus 기반 v17+로 이관하는 작업은 "메이저 업그레이드 + 런타임 교체"의 복합이다.
...
이 챕터로 Keycloak 실전 스터디 24개 챕터가 마무리된다. 처음에는 Realm 하나 만들어 로그인 받는 수준에서 출발해, 이제는 Operator로 HA 클러스터를 선언하고 Terraform으로 Realm을 관리하며 메이저 업그레이드까지 계획할 수 있는 지점에 이르렀다. 운영 원칙은 반복해서 강조했듯 단순하다.
