...
description: "ISOBUS 네트워크의 물리적 토폴로지, TBC 커넥터, ECU 종류, 메시지 흐름 순서를 이해합니다."
date: 2026-04-13
...
ISOBUS 네트워크는 물리적으로 두 개의 CAN 버스로 구성됩니다.
...
TBC는 트랙터와 작업기를 물리적으로 연결하는 9핀 커넥터입니다. ISO 11783-2에 정의되어 있습니다.
...
트랙터는 TBC를 통해 작업기 ECU에 전원을 공급합니다. ECU_PWR 핀을 통해 작업기의 ECU와 센서가 동작에 필요한 전력을 받습니다. 작업기 자체 전원(배터리)이 없어도 트랙터에 연결하면 즉시 동작 가능합니다.

---

## 3. ECU 종류

ISOBUS 네트워크 위에는 역할별로 명확히 구분된 ECU들이 존재합니다.
...
트랙터에 작업기를 연결하고 시동을 켠 후, ISOBUS 통신이 확립되기까지 일련의 단계가 순서대로 진행됩니다.
...
1. **주소 클레임**: 모든 ECU가 네트워크에서 고유한 주소를 확보합니다 (J1939 방식과 동일).
2. **Working Set 구성**: 작업기를 대표하는 WSM이 자신과 멤버 ECU를 선언합니다.
3. **VT 연결**: WSM이 VT에 연결을 요청하고 상태를 수신합니다.
4. **Object Pool 전송**: 작업기 UI 화면 데이터를 VT에 업로드합니다.
