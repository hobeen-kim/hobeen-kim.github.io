...
CAN 프레임의 데이터 필드는 **최대 8바이트**입니다. 단순한 센서 값이나 제어 신호는 8바이트 안에 담을 수 있지만, 다음과 같은 데이터는 그렇지 않습니다.
...
이런 데이터는 여러 CAN 프레임으로 **분할(segmentation)** 하여 순서대로 전송해야 합니다. 수신 측은 조각들을 모아 **재조립(reassembly)** 합니다. 이 역할을 담당하는 것이 **J1939 Transport Protocol(TP)**입니다.
...
BAM은 특정 수신자를 지정하지 않고 **버스 전체에 브로드캐스트**하는 방식입니다. 흐름 제어가 없으므로 수신자는 확인 응답(ACK)을 보내지 않습니다. 구현이 단순하지만 재전송이 불가능합니다.
...
Transport Protocol은 **두 가지 PGN**으로 동작합니다.

### TP.CM (PGN 60416, 0xEC00) — Connection Management

연결 관리를 담당합니다. BAM 공지, RTS/CTS 흐름 제어, EndOfMsg, Abort 메시지가 모두 이 PGN을 사용하며, **Control Byte(Byte 1)**로 메시지 종류를 구분합니다.
...
TP는 최대 **1785 바이트**(255 패킷 × 7 바이트)까지 전송할 수 있습니다. ISOBUS VT 오브젝트 풀처럼 더 큰 데이터를 전송하려면 **ETP(Extended Transport Protocol)**를 사용합니다.
...
ETP는 **CMDT 전용**입니다. BAM 방식의 ETP는 존재하지 않습니다. 이는 대용량 데이터 전송 시 흐름 제어 없이는 수신 버퍼 오버플로가 발생할 수 있기 때문입니다.
