import{_ as n,c as t,e as s,o as a}from"./app-m1b3yx1x.js";const l={};function i(o,e){return a(),t("div",null,e[0]||(e[0]=[s(`<h1 id="oauth는-무엇을-해결했는가" tabindex="-1"><a class="header-anchor" href="#oauth는-무엇을-해결했는가"><span>OAuth는 무엇을 해결했는가</span></a></h1><div class="hint-container info"><p class="hint-container-title">학습 목표</p><ul><li>권한 위임(delegation)의 개념과 기존 공유 모델과의 차이를 설명할 수 있다.</li><li>OAuth가 해결한 4가지 핵심 문제를 구체적인 시나리오로 열거할 수 있다.</li><li>Scope의 존재 이유와 세분화된 권한 부여가 왜 중요한지 이해한다.</li><li>사용자 동의(Consent) 화면의 역할과 OAuth의 한계(인증 아님)를 안다.</li></ul></div><h2 id="_1-대신-접근한다-는-아이디어" tabindex="-1"><a class="header-anchor" href="#_1-대신-접근한다-는-아이디어"><span>1. &quot;대신 접근한다&quot;는 아이디어</span></a></h2><p>OAuth의 핵심 아이디어는 놀랍도록 단순하다. <strong>&quot;비밀번호 대신, 제한된 권한만 담긴 증표(token)를 건넨다&quot;</strong>는 것이다. 이 문장 하나로 CH1에서 본 4대 문제가 거의 다 풀린다.</p><h3 id="전통적-방식과의-대비" tabindex="-1"><a class="header-anchor" href="#전통적-방식과의-대비"><span>전통적 방식과의 대비</span></a></h3><pre class="mermaid">flowchart LR
...
<span class="line"></span></code></pre><div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0;"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>또한 사용자는 AS의 계정 관리 페이지에서 &quot;연결된 앱&quot; 목록을 보고 원하는 앱의 권한을 한 번에 취소할 수 있다.</p><pre class="mermaid">flowchart LR
    ...
    style INV fill:#fee
</pre><p>중요한 것은 <strong>비밀번호를 바꾸지 않아도 된다</strong>는 점이다. 앱 A만 연결을 끊으면 앱 B, C는 정상 동작을 유지한다. 이 입도(granularity)는 비밀번호 공유 시대에는 불가능했다.</p><h3 id="문제-4-—-감사-audit-가능" tabindex="-1"><a class="header-anchor" href="#문제-4-—-감사-audit-가능"><span>문제 4 — 감사(Audit) 가능</span></a></h3><p>OAuth에서 모든 API 호출은 <strong>어떤 토큰으로</strong> 들어왔는지 식별된다. 토큰에는 다음 정보가 결합되어 있다.</p><ul><li>사용자(Resource Owner)</li><li>클라이언트(Client ID)</li><li>Scope</li><li>발급 시각·만료 시각</li><li>세션/디바이스 정보 (확장)</li></ul><p>자원 서버의 로그를 분석하면 &quot;사용자 42가 2026-04-17 10:23:45에 <strong>앱 Notion</strong>에게 <code>drive.readonly</code> 권한을 위임했고, 그 토큰으로 <code>/drive/files/xxx</code>를 조회했다&quot;처럼 책임 소재가 명확하게 추적된다.</p><p>비정상 패턴 탐지도 가능하다. 같은 토큰이 갑자기 해외에서 사용된다거나, 평소와 다른 Scope를 호출한다거나 하는 이상 행동을 토큰 단위로 감지할 수 있다.</p><h2 id="_3-사용자-동의-consent-화면의-역할" tabindex="-1"><a class="header-anchor" href="#_3-사용자-동의-consent-화면의-역할"><span>3. 사용자 동의(Consent) 화면의 역할</span></a></h2><p>&quot;앱이 당신의 구글 캘린더에 접근해도 될까요?&quot; 우리가 익숙한 이 화면이 Consent(동의) 화면이다. OAuth의 설계에서 Consent는 <strong>법적·윤리적 장치</strong>의 역할도 겸한다.</p><h3 id="consent-화면의-네-가지-기능" tabindex="-1"><a class="header-anchor" href="#consent-화면의-네-가지-기능"><span>Consent 화면의 네 가지 기능</span></a></h3><pre class="mermaid">flowchart TD
...
<span class="line"></span></code></pre><div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0;"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h2 id="_5-oauth의-한계" tabindex="-1"><a class="header-anchor" href="#_5-oauth의-한계"><span>5. OAuth의 한계</span></a></h2><p>OAuth가 모든 걸 해결한 것은 아니다. CH3에서 이미 짚었듯, OAuth는 <strong>인증을 책임지지 않는다</strong>. 이 한계는 OIDC로 보완된다.</p><h3 id="oauth-혼자-할-수-없는-것" tabindex="-1"><a class="header-anchor" href="#oauth-혼자-할-수-없는-것"><span>OAuth 혼자 할 수 없는 것</span></a></h3><ol><li><strong>사용자 로그인(인증)</strong> — Access Token으로 사용자 신원을 확정하는 것은 위험하다.</li><li><strong>사용자 정보 표준화</strong> — &quot;이름&quot;, &quot;이메일&quot;을 담는 표준 필드가 없다. 각 IdP가 임의 정의한다.</li><li><strong>세션 관리</strong> — 싱글 로그아웃(Single Logout), 세션 타임아웃 표준이 없다.</li><li><strong>토큰 포맷 규정</strong> — Access Token이 opaque인지 JWT인지, 어떤 필드를 담는지 명시되지 않는다.</li></ol><h3 id="oidc는-이-공백을-채운다" tabindex="-1"><a class="header-anchor" href="#oidc는-이-공백을-채운다"><span>OIDC는 이 공백을 채운다</span></a></h3><pre class="mermaid">flowchart TB
