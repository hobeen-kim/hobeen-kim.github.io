import{_ as e,c as n,d as a,o as s}from"./app-CJC3hE05.js";const r={};function d(l,t){return s(),n("div",null,t[0]||(t[0]=[a(`<h1 id="j1939-transport-protocol" tabindex="-1"><a class="header-anchor" href="#j1939-transport-protocol"><span>J1939 Transport Protocol</span></a></h1><h2 id="학습-목표" tabindex="-1"><a class="header-anchor" href="#학습-목표"><span>학습 목표</span></a></h2><ul><li>CAN 프레임의 8바이트 제한과 Transport Protocol의 필요성을 설명할 수 있다.</li><li>BAM 방식과 CMDT 방식의 차이점과 적용 시나리오를 이해한다.</li><li>TP.CM과 TP.DT 메시지의 역할과 PGN 번호를 안다.</li><li>ETP가 필요한 상황과 TP와의 차이를 설명할 수 있다.</li><li>타임아웃 규칙을 이해하고 Abort 처리 흐름을 설명할 수 있다.</li></ul><hr><h2 id="_1-왜-transport-protocol이-필요한가" tabindex="-1"><a class="header-anchor" href="#_1-왜-transport-protocol이-필요한가"><span>1. 왜 Transport Protocol이 필요한가</span></a></h2><p>CAN 프레임의 데이터 필드는 <strong>최대 8바이트</strong>입니다. 단순한 센서 값이나 제어 신호는 8바이트 안에 담을 수 있지만, 다음과 같은 데이터는 그렇지 않습니다.</p><table><thead><tr><th>데이터 유형</th><th>일반 크기</th></tr></thead><tbody><tr><td>DM1 (진단 코드 목록)</td><td>수십~수백 바이트</td></tr><tr><td>VT 오브젝트 풀 (화면 정의)</td><td>수십 KB ~ 수 MB</td></tr><tr><td>소프트웨어 업데이트 펌웨어</td><td>수 KB ~ 수 MB</td></tr><tr><td>제품 식별 정보(Product ID)</td><td>수십 바이트</td></tr></tbody></table><p>이런 데이터는 여러 CAN 프레임으로 <strong>분할(segmentation)<strong> 하여 순서대로 전송해야 합니다. 수신 측은 조각들을 모아 </strong>재조립(reassembly)</strong> 합니다. 이 역할을 담당하는 것이 <strong>J1939 Transport Protocol(TP)</strong>입니다.</p><pre class="mermaid">graph TD
...
</pre><h3 id="cmdt-제어-메시지-control-byte-값" tabindex="-1"><a class="header-anchor" href="#cmdt-제어-메시지-control-byte-값"><span>CMDT 제어 메시지 Control Byte 값</span></a></h3><table><thead><tr><th>메시지</th><th>Control Byte</th><th>설명</th></tr></thead><tbody><tr><td>RTS</td><td>0x10</td><td>전송 요청 (총 크기, 패킷 수, PGN 포함)</td></tr><tr><td>CTS</td><td>0x11</td><td>수신 준비 (허용 패킷 수, 시작 시퀀스 번호)</td></tr><tr><td>EndOfMsgACK</td><td>0x13</td><td>전체 전송 완료 확인</td></tr><tr><td>Abort</td><td>0xFF</td><td>연결 중단 (오류 코드 포함)</td></tr></tbody></table><h3 id="bam-vs-cmdt-비교" tabindex="-1"><a class="header-anchor" href="#bam-vs-cmdt-비교"><span>BAM vs CMDT 비교</span></a></h3><table><thead><tr><th>항목</th><th>BAM</th><th>CMDT</th></tr></thead><tbody><tr><td>수신 대상</td><td>브로드캐스트 (전체)</td><td>특정 장치 (1:1)</td></tr><tr><td>흐름 제어</td><td>없음</td><td>있음 (CTS)</td></tr><tr><td>완료 확인</td><td>없음</td><td>EndOfMsgACK</td></tr><tr><td>재전송</td><td>불가</td><td>가능 (Abort 후 재시도)</td></tr><tr><td>최대 크기</td><td>1785 byte</td><td>1785 byte</td></tr></tbody></table><hr><h2 id="_4-tp-cm과-tp-dt" tabindex="-1"><a class="header-anchor" href="#_4-tp-cm과-tp-dt"><span>4. TP.CM과 TP.DT</span></a></h2><p>Transport Protocol은 <strong>두 가지 PGN</strong>으로 동작합니다.</p><h3 id="tp-cm-pgn-60416-0xec00-—-connection-management" tabindex="-1"><a class="header-anchor" href="#tp-cm-pgn-60416-0xec00-—-connection-management"><span>TP.CM (PGN 60416, 0xEC00) — Connection Management</span></a></h3><p>연결 관리를 담당합니다. BAM 공지, RTS/CTS 흐름 제어, EndOfMsg, Abort 메시지가 모두 이 PGN을 사용하며, <strong>Control Byte(Byte 1)</strong>로 메시지 종류를 구분합니다.</p><div class="language-text line-numbers-mode" data-highlighter="prismjs" data-ext="text" data-title="text"><pre><code><span class="line">TP.CM 메시지 (8 byte):</span>
...
<span class="line">└────────────────────────────────────────────────────┘</span>
<span class="line"></span></code></pre><div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0;"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h3 id="tp-dt-pgn-60160-0xeb00-—-data-transfer" tabindex="-1"><a class="header-anchor" href="#tp-dt-pgn-60160-0xeb00-—-data-transfer"><span>TP.DT (PGN 60160, 0xEB00) — Data Transfer</span></a></h3><p>실제 데이터를 7바이트씩 분할하여 전송합니다. <strong>Byte 1은 시퀀스 번호(1~255)</strong>, Byte 2~8이 페이로드입니다. 마지막 패킷의 남는 바이트는 0xFF로 채웁니다.</p><div class="language-text line-numbers-mode" data-highlighter="prismjs" data-ext="text" data-title="text"><pre><code><span class="line">TP.DT 메시지 (8 byte):</span>
<span class="line">┌──────────────────────────────────────────┐</span>
...
</pre><p>ETP는 <strong>CMDT 전용</strong>입니다. BAM 방식의 ETP는 존재하지 않습니다. 이는 대용량 데이터 전송 시 흐름 제어 없이는 수신 버퍼 오버플로가 발생할 수 있기 때문입니다.</p><h3 id="isobus-vt-오브젝트-풀-전송" tabindex="-1"><a class="header-anchor" href="#isobus-vt-오브젝트-풀-전송"><span>ISOBUS VT 오브젝트 풀 전송</span></a></h3><p>VT(Virtual Terminal)에 화면을 표시하려면 오브젝트 풀을 전송해야 합니다. 오브젝트 풀은 수십 KB를 초과하는 경우가 많으므로 ETP를 통해 전송됩니다.</p><div class="language-text line-numbers-mode" data-highlighter="prismjs" data-ext="text" data-title="text"><pre><code><span class="line">작업기 ECU → VT:</span>
