...
#### 교차 링크 규칙
- 관련 블로그 포스트(`/posts/...`)나 다른 스터디 챕터(`/study/...`)가 있으면 본문에 링크를 삽입한다
- 예: `자세한 내용은 [Lateral Join 포스트](/posts/database/2025-04-20-leteraljoin)를 참고한다.`
- 예: `::: info 관련 스터디` 컨테이너로 `[ISOBUS CH6](/study/isobus/06-can-error)에서 상세히 다룬다.`

#### 문체 규칙
- **반말 평서체(해라체)** 사용 — `~이다`, `~한다`, `~있다`, `~된다` 등. `~입니다`, `~합니다`, `~있습니다` 등 경어체 금지

#### 섹션 구분 규칙
- `##` 섹션 사이에 `---`(horizontal rule) 넣지 않음 — frontmatter 구분자 2개만 사용
- 섹션 전환은 제목(`##`)만으로 충분하며, 불필요한 수평선은 피함
