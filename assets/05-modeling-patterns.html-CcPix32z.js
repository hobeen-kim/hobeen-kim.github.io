...
<span class="line">  relation viewer: user | folder#viewer | user:*</span>
...
<span class="line">  permission view = viewer + edit + parent-&gt;view</span>
<span class="line">}</span>
<span class="line"></span></code></pre><div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0;"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><p>핵심 포인트를 짚어 보자.</p><p>첫째, <code>parent: folder</code>가 폴더 트리를 만든다. <code>permission edit = ... + parent-&gt;edit</code>는 &quot;내 부모 폴더의 edit 권한이 있으면 나도 edit 가능&quot;이라는 재귀적 정의다. SpiceDB는 이걸 arrow 재귀로 풀어내면서 순환이 없도록 탐색한다.</p><p>둘째, <code>editor: user | folder#editor</code>에서 <code>folder#editor</code> 부분은 &quot;다른 폴더의 editor 전원&quot;을 subject로 받는 문법이다. &quot;이 공유 폴더의 editor는 팀 공용 폴더의 editor와 동일&quot; 같은 위임 관계를 만들 때 쓴다.</p><p>셋째, <code>viewer: user | ... | user:*</code>의 <code>user:*</code>가 바로 &quot;링크 있는 누구나&quot;를 표현하는 wildcard다. 이 relation에 <code>folder:shared#viewer@user:*</code>를 하나 써두면 모든 user가 viewer로 취급된다.</p><p>넷째, 파일의 <code>permission view = viewer + edit + parent-&gt;view</code> 순서가 중요하다. <code>edit</code>이 먼저 들어가면, 개별 파일 edit 권한이 있는 사람은 자동으로 view도 받는다. <code>parent-&gt;view</code>는 폴더의 뷰어 상속이다.</p><div class="hint-container warning"><p class="hint-container-title">user:* wildcard는 검토가 두 번 필요하다</p><p><code>user:*</code>는 편하지만 실수로 공개 공유가 되기 쉽다. Schema 리뷰와 relationship 쓰기 양쪽에서 공개 의도가 맞는지 반드시 확인한다. 가능하면 공개용 relation은 별도 이름(예: <code>public_viewer</code>)으로 분리해서 눈에 띄게 만드는 편이 안전하다.</p></div><pre class="mermaid">flowchart TB
  U[user:alice]
  F1[folder:root]
  F2[folder:project]
  FIL[file:spec.md]
