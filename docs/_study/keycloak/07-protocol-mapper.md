...
prev: /study/keycloak/06-role-group
next: /study/keycloak/08-authz-uma
---
...
Script Mapper는 유연하지만 기본적으로 비활성화된 Preview 기능이다. 활성화하려면 서버 시작 시 `--features=scripts` 플래그를 켜야 하고, 보안상 JS 실행 환경은 번들된 Nashorn 기반이라 ECMAScript 버전과 성능 제약이 있다. 운영 환경에서는 [CH15 SPI](/study/keycloak/15-spi-overview)에서 다룰 커스텀 Mapper Provider로 대체하는 것이 일반적이다.
...
Users → 대상 사용자 → Attributes 탭에서 `department = "platform"`, `employee_number = "A12345"`를 추가한다. 관리 자동화 관점에서는 Admin REST API([CH22](/study/keycloak/22-admin-rest-api) 예고)로 일괄 주입하는 쪽이 맞다.
...
- <strong>모든 토글을 켜는 것</strong>: 토큰이 부풀고 민감정보가 불필요하게 유통된다.
- <strong>Access Token에 `email` 담기</strong>: 이메일은 권한 판단에 필요한 정보가 아니다. API가 식별이 필요하면 `sub`로 충분하고, 표시용 이름은 UserInfo에서 가져온다.
- <strong>ID Token에 Role 담기</strong>: ID Token은 "로그인 사실 증명"이지 "권한 주장"이 아니다. Role은 Access Token에 담는 것이 맞다.
...
`email` Scope는 `email`·`email_verified` 두 Mapper로, `address` Scope는 `address` 복합 객체 Mapper로 구성된다. 이 기본 Mapper들은 삭제보다 <strong>Include in 토글</strong>을 끄는 방식으로 제어하는 것이 안전하다.
...
- 기존 Scope에 Mapper를 추가할 때 <strong>ID Token/Access Token 크기 증가</strong>가 임계치를 넘는지 미리 측정.
...
- 이전 : [Role·Group과 Composite Role](/study/keycloak/06-role-group)
- 다음 : [Authorization Services와 UMA 2.0](/study/keycloak/08-authz-uma)
