...
prev: /study/keycloak/05-role-group
next: /study/keycloak/07-authz-uma
---
...
Script Mapper는 유연하지만 기본적으로 비활성화된 Preview 기능이다. 활성화하려면 서버 시작 시 `--features=scripts` 플래그를 켜야 하고, 보안상 JS 실행 환경(Nashorn→Rhino 등)의 제약이 있다. 운영 환경에서는 [CH14 SPI](/study/keycloak/14-spi-overview)에서 다룰 커스텀 Mapper Provider로 대체하는 것이 일반적이다.
...
Users → 대상 사용자 → Attributes 탭에서 `department = "platform"`, `employee_number = "A12345"`를 추가한다. 관리 자동화 관점에서는 Admin REST API([CH21](/study/keycloak/21-admin-rest-api) 예고)로 일괄 주입하는 쪽이 맞다.
...
- 이전 : [Role·Group과 Composite Role](/study/keycloak/05-role-group)
- 다음 : [Authorization Services와 UMA 2.0](/study/keycloak/07-authz-uma)
